...
| Investigation | Implemented foundation | Persistent facts, a strict mTLS READ Task Gateway, an atomic connector/target/egress/executor Bundle, a recovery-first Temporal v2 Workflow, fixed READ Runner Activity, immutable assembly Snapshot, role-isolated Temporal Starter/Control Worker, monitored fatal/stop subprocess containment, a sealed pre-assembly child lifecycle arbiter, and a kill-bounded fixed-root public-source loader plus sealed FD4 handoff exist; the child now constructs the semantic Snapshot solely from that validated envelope, resolves target CAs from the captured closure, and exactly compares `expected_snapshot`, while the fixed factory still prevents secret reads, Dial and READY, live Outbox/Runner assembly is absent, and the non-configurable Admission remains closed pending M5C2-4c and external Go/No-Go gates |
