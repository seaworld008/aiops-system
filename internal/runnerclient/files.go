...
func openTrustFile(path string, privateKey bool) (*openedTrustFile, error) {
	...
	file := os.NewFile(uintptr(fd), "runner-client-trust-file")
	if file == nil {
		_ = syscall.Close(fd)
		return nil, ErrInvalidConfiguration
	}
...
func (opened *openedTrustFile) readStable() ([]byte, error) {
	...
	if err != nil || !validTrustFileInfo(after, opened.privateKey) || trustFileHasExtendedMetadata(opened.file) ||
		...
		!after.ModTime().Equal(opened.info.ModTime()) || !bytes.Equal(first, second) {
		clear(first)
		return nil, ErrInvalidConfiguration
	}
	return first, nil
...
// check stays bound to the already-open descriptor and fails closed whenever
// enumeration is unavailable. Only kernel provenance/integrity labels that
// cannot expand access are permitted.
