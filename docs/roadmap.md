...
| Temporal orchestration | Digest/Bundle-bound READ runtime v2, DB-only result recovery, strict payload/failure History allowlist, server-attested separate Starter/Control mTLS capabilities, fixed Control Worker, and the C2-4c2b0 monitored TERM/FATAL, deterministic kill/reap, sealed pre-assembly child lifecycle arbiter, and static gates against unreviewed process creation/group escape/namespace primitives implemented; repository-wide raw syscall checks preserve only exact reviewed metadata/hardening callsites. This proves bounded process-group containment, not SDK Stop concurrency or cgroup isolation, and the hidden child still deliberately exits before READY | Put the real Control Worker and clients inside the contained child, require cluster-exclusive reconnect identity and cgroup/PID-namespace evidence, then complete supervised Outbox/Runner/Gateway assembly and prove separate Temporal credentials/RBAC |
