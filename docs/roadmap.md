...
| Temporal orchestration | Digest/Bundle-bound READ runtime v2, DB-only result recovery, strict History allowlist and runner-only Activity implemented but not process-assembled | Complete live Outbox/Worker/Runner/Gateway assembly, then the evidence/model workflow and approval/execution workflows |
