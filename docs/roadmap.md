...
| Investigation and model routing | Persistent runtime, authenticated READ Task Gateway, atomic runtime Bundle, independent READ-only client, fixed READ HTTP executor, recovery-first Temporal v2, READ Runner Activity, immutable Snapshot and role-isolated Temporal control boundary implemented but not process-assembled; a sealed Admission blocks both new claims and legacy lease progression | Atomically assemble Runner/Worker/Gateway/Outbox and local E2E in M5C2-4c, then pass external identity/network gates and build a replay set of at least 100 historical incidents |
...
| Temporal orchestration | Digest/Bundle-bound READ runtime v2, DB-only result recovery, strict payload/failure History allowlist, server-attested separate Starter/Control mTLS capabilities and fixed Control Worker implemented but not process-assembled | Clear the C2-4c2 external gates: cluster-exclusive reconnect identity plus subprocess deadline/kill/reap containment for SDK Start/Stop/fatal overlap; then complete supervised Outbox/Worker/Runner/Gateway assembly and prove separate Temporal credentials/RBAC |
