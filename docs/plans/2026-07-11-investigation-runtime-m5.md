...
日期：2026-07-11
范围：M5A1 领域契约与 Memory fixture、M5A2a PostgreSQL 运行时迁移、M5A2b 事务型 PostgreSQL Repository、M5B1 READ Runner 持久 attempt/fence 底座，以及 M5B2 独立 READ Task Gateway 协议
...
- M5B2 的 Control Plane 装配有意保持 READ claims disabled：协议和事务边界可验证，但在 M5C 注入不可变的 ConnectorID/Operation typed input/output registry 前不创建 lease。M5C 必须接通 Temporal/Outbox，并让 Workflow/History 只传 ID 和小型脱敏 receipt；连接目标由 Gateway 服务端配置解析，不接受 TaskSpec 注入目标或凭据。通用敏感词黑名单不能替代 schema allowlist 这一主控制。
- M5D 必须实现模型路由、结构化 Hypothesis 校验、确定性证据报告、人工 Feedback 与离线评测。模型失败和无模型配置不得吞掉 Evidence，也不得升级为生产动作授权。

## 生产写不存在启用路径

M5A1–M5B2 增加领域/Memory fixture、调查事实 schema、READ attempt/fence、PostgreSQL 持久化闭环和仅位于 `:8443` mTLS listener 的受信 READ Task Gateway；它不包含 Temporal Workflow，也不是公共 HTTP API。Control Plane 尚未注入真实 typed connector registry，因此 READ claim 仍 fail closed。调查结果没有连接到 ActionPlan、策略、审批、执行器或 production-write feature flag；生产写继续不存在可配置启用路径。
