...
- Manifest/registry 文件共用 fail-closed secure loader：干净绝对路径、最终组件不跟随 symlink、当前 euid 所有的单 hardlink regular file、非 group/world writable、最大 1 MiB、FD 双读稳定性、ACL/未知 xattr 拒绝，以及 UTF-8/深度 16/规范 snake_case/未知和转义重复字段/尾随文档拒绝。内存构造也在深拷贝或 JCS 前执行溢出安全的 1 MiB 聚合定义预算；错误不得泄露路径或内容。
