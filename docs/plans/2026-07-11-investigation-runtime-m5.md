...
- M5C1B0 没有数据库迁移、Temporal SDK、History DTO、task queue、`cmd/*` 配置或 assembly。后续 B1 History DTO 必须只携带持久 ID、脱敏状态和四个摘要，Activity queue 必须与 plan/registry digest 绑定；旧 digest 的兼容 Worker 必须保留到对应 Workflow/READ Task 全部终态，不能让新定义重新解释旧 History。上述 B1 rollout 约束目前只是后续门禁，不得描述为已装配能力。
...
M5A1–M5C1B0 增加领域/Memory fixture、调查事实 schema、READ attempt/fence、PostgreSQL 持久化闭环、仅位于 `:8443` mTLS listener 的受信 READ Task Gateway、尚未接入 live runtime 的 typed connector registry、未装配的 exact-type Outbox 路由底座，以及纯内存 immutable investigation Planner；M5C1B0 自身不新增数据库迁移，也不包含 Temporal SDK/Workflow、History/task queue 或公共 HTTP API。Control Plane 仍安装 disabled callbacks，`cmd/*` 与 Worker 未安装 signal dispatcher，因此 READ claim、遗留 lease 和未 ACK Outbox 均 fail closed。调查结果没有连接到 ActionPlan、策略、审批、执行器或 production-write feature flag；生产写继续不存在可配置启用路径。
