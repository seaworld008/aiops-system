...
日期：2026-07-11
范围：M5A1 领域契约与 Memory fixture，以及 M5A2a PostgreSQL 运行时迁移
...
- Memory 为了测试作用域碰撞、ID factory 冲突和别名隔离，允许使用符合领域语法的普通字符串 ID；现有 PostgreSQL 核心表以 `uuid` 为主键，因此 PostgreSQL adapter 必须在进入 SQL 前把所有持久化资源 ID 收窄为合法 UUID。它仍必须在每条查询和复合外键中携带 Tenant/Workspace，不能把 UUID 的全局唯一性当作租户授权。
...
Hypothesis `confidence` 到遗留 `confidence_band` 的映射固定为：`[0, 0.5) → LOW`、`[0.5, 0.8) → MEDIUM`、`[0.8, 1] → HIGH`。迁移 CHECK 与后续 PostgreSQL adapter 必须共用这组边界，禁止调用方直接提供或覆盖 band。
...
- M5B 必须在 `000010` 之上提供事务型 PostgreSQL Repository，实现与本文件领域契约一致的 Workspace 隔离、唯一活动调查、幂等冲突、Evidence/Hypothesis 引用完整性和 Outbox 原子提交；Memory 行为不能作为持久化替代。
- M5C 必须在 Temporal/Runner Gateway 边界实现任务领取、lease fencing、重试与取消。Workflow/History 只传 ID 和小型脱敏 receipt；连接器目标由 Gateway 服务端配置解析，不接受 TaskSpec 注入目标或凭据。Gateway 只有按已登记 ConnectorID/Operation 的 output schema 完成 typed projection 与 redaction 后，才能调用领域 `CompleteTask`；通用敏感词黑名单只能作为纵深防御，不能替代 schema allowlist 这一主控制。
- M5D 必须实现模型路由、结构化 Hypothesis 校验、确定性证据报告、人工 Feedback 与离线评测。模型失败和无模型配置不得吞掉 Evidence，也不得升级为生产动作授权。

## 生产写不存在启用路径

M5A1–M5A2a 只增加领域/Memory fixture 与调查事实的 PostgreSQL schema，不包含 Temporal Workflow、调查 Runner Gateway 路由或 HTTP API；也没有把调查结果连接到 ActionPlan、策略、审批、执行器或 production-write feature flag。Memory 中的 `FailInvestigation`、任务取消、Feedback 与其他写方法都只是 fixture 内存状态转换，不能触发连接器调用或任何生产写操作。后续里程碑在完整策略、审批、短期凭据、隔离 Runner 和执行审计链全部落地前，必须继续保持生产写不可达。
