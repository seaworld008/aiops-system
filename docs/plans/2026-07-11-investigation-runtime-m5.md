...
- M5C1A 只建立 Outbox 路由与启动边界底座：Repository 在数据库 `ORDER/LIMIT/FOR UPDATE` 之前按单一 exact `event_type` 过滤，并把同一类型绑定到 Ack/Retry fence；`SignalDispatcher` 固定只接受 `signal.ingested.v1`，只把持久 UUID 与版本事实投影为 secret-safe start DTO。`000012` 仅增加通用 pending exact-type partial index，不替换既有索引。此阶段不引入 Temporal SDK、不实现或安装 placeholder Workflow，也不修改任何 `cmd/*`、Control Plane 或 Worker 装配。
