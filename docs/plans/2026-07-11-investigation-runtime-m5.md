...
日期：2026-07-11
范围：M5A1 领域契约与 Memory fixture、M5A2a PostgreSQL 运行时迁移、M5A2b 事务型 PostgreSQL Repository，以及 M5B1 READ Runner 持久 attempt/fence 底座
...
- READ 完成正文只能包含有界结构化 Evidence items 或固定低敏失败码，不存在 Runner 可控的 `truncated` 信任位；无法满足完整 typed connector 契约时必须把该 Task 作为失败返回，由多个 Task 结果推导 `PARTIAL` Investigation。Connector、Operation、Workspace、Environment、Runner、scope revision、证书、item count、幂等键和所有 SHA-256 均由已持久化事实与服务端 JCS 投影生成；正文不能提交 source、目标 URL、header、credential、原始错误体或自选 hash。Connector/Operation 的最终 typed output allowlist 仍由 M5B2 Gateway 注入，通用安全 JSON 校验仅作为纵深防御。
- `internal/readtask/postgres` 是 caller-owned transaction 的低层组件，不是独立授权边界。调用者必须先在同一事务执行 `runneridentity/postgres.AuthenticateTx`，再调用 READ attempt 操作，并在任一错误时回滚；`Start` 只保留强制 `StartRunnerAuthorizedTx` 入口。数据库触发器会再次校验当前精确身份，允许身份漂移后的 fail-safe `CANCELLED/EXPIRED`，并按 identity → Task → attempt → Investigation 的顺序取锁后才读取安全时间。
...
- M5B1 已在 `000010` 和事务型 PostgreSQL Repository 之上建立 READ Runner attempt、lease fencing、严格 heartbeat、GO 前 release、重试与 Evidence/receipt 原子事务。通用 `Repository.CompleteTask` 继续 fail closed。M5B2 仍必须新增独立于 WRITE `/jobs*` 的严格 READ task Gateway 协议，并只从每请求 mTLS 数据库认证派生 scope/证书后调用该事务入口；不得把仓储方法直接暴露给公共 HTTP。
...
M5A1–M5B1 只增加领域/Memory fixture、调查事实 schema、READ attempt/fence 与 PostgreSQL 持久化闭环，不包含 Temporal Workflow、受信调查 Runner Gateway 路由或公共 HTTP API；也没有把调查结果连接到 ActionPlan、策略、审批、执行器或 production-write feature flag。PostgreSQL 的通用任务完成入口明确 fail closed，不能从请求正文接受 Runner 身份。后续里程碑在完整策略、审批、短期凭据、隔离 Runner 和执行审计链全部落地前，必须继续保持生产写不可达。
