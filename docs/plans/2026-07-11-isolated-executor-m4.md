...
Runner 与 Executor 在读取任何凭据前均设置并读回验证 `RLIMIT_CORE=0`、
`PR_SET_DUMPABLE=0` 和 `PR_SET_NO_NEW_PRIVS=1`，任一失败即退出。子进程只继承协议
FD，使用空的 `0700` 作业目录和固定环境白名单；stdout/stderr 合计超过
