...
Phase 1 的稳定接口进一步锁定：Source/Asset revision 使用 `int64`；`asset_source_revisions.canonical_revision_digest` 就是覆盖完整不可变可用性绑定的 `BindingDigest`，`source_definition_digest` 是 `asset-source-definition.v2` 的 Provider/Profile definition（包含持久 canonical Profile manifest 与 Provider schema 的 raw SHA，不包含 source-specific binding 或 typed extension）；资产类型在 `000015` 以命名闭集约束 `assets_kind_check` 建立并由 `000017` 扩展；跨 Environment Relationship 显式携带 source/target Environment；Binding 软删除状态为 `INACTIVE`；所有 Mapping mutation 都返回持久 `MutationReceipt`。Source Gate qualification 的实现顺序固定为 `Task 28C → Task 19A → Task 19A2a → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`：profile/validation admission、12-file schema/domain/admission、11-file persistence/runtime rechecks、19-file qualification API/production assembly、provider-neutral HA receipt/metrics wiring、per-source Provider canary+atomic gate、最终 matrix 各有唯一 owner；每个 Batch 都由新窗口独立真实 G2/PR/merge，所有后继只消费已合并 `Produces`。Task 19A2c 必须复用 Task 28A 的唯一 Worker loop；若其 stable seam 不足，先停工并以只修改 `worker.go/worker_test.go/claim_runtime.go/claim_runtime_test.go` 的 sequential C0 corrective 扩展同一 mode/outcome sink，禁止第二 runner。Task 28C 只在 production constructor 冻结 observer/decorator extension seam；Task 29A 若预检该 seam 或 19A2c verifier registration 不足，先停并做冻结的六文件 sequential C0 corrective，不能靠脚本、日志或 fake 伪造 signed HA receipt/metrics。后续任务包只能消费这些定义，不得重定义平行 DTO、摘要语义、开门证据或软删除状态。
