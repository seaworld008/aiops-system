...
当前唯一执行顺序为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。global routine ACL corrective把原 exact 8 docs 与`docs/status/current.md`、`coverage-matrix.md`、Pack 09 作为同一 exact11 原子提交，进入`main`时合同与当前入口同时生效，不再安排后继 manager sync，也不改 ABI、业务完成度、checkbox或能力状态。上一 fresh A2a 因 predecessor72 默认 PUBLIC `EXECUTE` 为两个 capability identity产生合计144条额外 effective edge而`STOPPED/NOT PASS`；其未提交 worktree不得续用或复制，本exact11不实现A2a/G2。pre-A2a exact-2 的 global helper保持安全真值；exact11合并后的唯一下一入口是从届时最新`origin/main`新建fresh A2a exact-12，消费Pack06唯一exact72 manifest与production digest，并负责pre-up72、up110 direct/effective/owner/grantor/grantability、runtime exact72 edge digest、unexpected111、wrong predecessor ACL、down72 restored、up/down/up、五身份和required双实例dump/restore RED→GREEN，以application identity运行既有关键DML、trigger与四条helper路径证明hardening不回归普通应用。每步只消费最新`main`已合并`Produces`；Task19A2c必须复用Task28A唯一Worker loop，Task29A若seam不足先做冻结corrective。所有 Provider/Worker在真实 G3/G4前继续`NOT_STARTED/UNAVAILABLE/CLOSED`。
