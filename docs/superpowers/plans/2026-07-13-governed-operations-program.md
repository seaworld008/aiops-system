...
Source Gate qualification 逐字消费规范 [§5.2.1](../specs/2026-07-13-operational-assets-controlled-access-design.md#521-source-gate-qualification-唯一持久-abi)：`asset_sources` exactly 3 nullable pointer columns，`asset_source_runs` exactly 13 qualification + 10 HA nullable columns，named four-column deferred identity FK与 exact 15-frame receipt。`000015` 在既有 36 routines 上只新增 canonical `public.asset_catalog_seal_qualification_receipt(uuid,uuid,uuid,uuid,bigint,bigint,text,timestamp with time zone,timestamp with time zone,text)` 和 `public.asset_catalog_admit_source_gate(uuid,uuid,uuid,uuid,bigint,bigint)`。普通 runtime/workload均无 EXECUTE；独立无 membership/关系权限的 `aiops_source_gate_sealer` only seal、`aiops_source_gate_admitter` only admit，函数另验 exact session user。两者固定 target Run→receipt/UUID-ordered prior Runs→Source→published Revision锁序与 exact CAS，Audit/Outbox append last；`RecordCleanup`先持久 `REVOKED`，first-write seal再要求同一 transaction的 DB issued time、不早于 cleanup receipt与 `expiry<=issued+24h`，并派生 receipt/HA/terminal/capacity/fence closure和 fixed terminal Audit；已封存 exact tuple只读 replay，changed replay拒绝。admit只在 A2b/A2c current trust复验后派生 pointer/open Audit/Outbox。A2a只证明 structural closure，不宣称密码学验签或 current Profile/runtime/key。Source/Run protected direct DML仍按 exact 3+23/7+16边界返回`42501`；exact MANUAL三元特例与 mixed/missing `IS TRUE` fail closed只由有 published Revision事实的 DB/A2b门判断。

Routine manifest 分为两层：A2a 后 `000015` owned manifest 是 exact38（existing36+new2），application schema global exact110=predecessor72+Asset38。静态基线是78次定义、6次replacement、最终72 identity（68 trigger+4 helper），且predecessor没有direct grant而依赖normalized PUBLIC。唯一C-order identities与production digest见 [Pack06 canonical predecessor exact72 runtime EXECUTE manifest](./2026-07-13-governed-operations/01-assets/06-source-external-cmdb.md#canonical-predecessor-exact72-runtime-execute-manifest)；本总计划不复制列表。Migration/admission/test必须显式逐项匹配该列表并使用该production常量，不得运行时推导后接受；未知73rd、缺失、overload或ACL/owner/grantor/grantability drift在DDL前整笔回滚。up显式revoke predecessor72与owned38 PUBLIC并grant runtime exact72 owner-grantor/non-grantable edges，禁止`ON ALL FUNCTIONS IN SCHEMA`；global110后PUBLIC0、owner110、runtime direct/effective90、workload direct0/effective90、sealer/admitter各1。down先revoke新增runtime72，再删除owned38并恢复PUBLIC72，最终catalog/ACL等于pre-up；禁止schema-wide grant/revoke或恢复未知对象。未来`000016..000022`新增/替换public routine必须更新global manifest、显式revoke与rollback contract，否则admission关闭。

当前唯一执行顺序为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → pre-A2a identity-FK fixture compatibility exact-9B docs-only contract checkpoint → [DEVELOPMENT_PAUSED] → resumed fresh test-only identity-FK fixture corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。global routine ACL exact11 已由 PR #152 合并为 `main@d8d632f453929fbe056dc499a6d64144d83011bf`；formal-fixture exact9 已由 PR #153 合并为 `main@c0b620e6fff9de0b746504f5fb7231fcb4a213c4`、tree `0af13374448f1386593291631e10a07add41440b`。此前 fresh A2a 分别因 predecessor72 默认 PUBLIC `EXECUTE` 的额外 capability edges与exact3/trigger/drop fixture冲突而停止；PR #153 后的新 fresh formal A2a 又确认merged fixture必然拒绝合法identity FK的top-level与inline两种表达。全部 partial A2a 都是`STOPPED/NOT PASS`，停止的`cb00`及其他 dirty/stopped A2a worktree/WIP/snapshot永不作为输入，任何未完成实现都不提交、不合并。

截至2026-07-23用户决定暂停开发；当前 Batch 只把 pre-A2a identity-FK fixture compatibility exact9B 的八份权威文档冻结为 docs-only 暂停检查点，原 Phase B test-only实现永久停止于本轮并保持`NOT_STARTED`，不是本检查点或其后文档PR的交付。冻结的未来fixture parser/extractor合同只识别inline named constraint或唯一精确top-level`ALTER TABLE public.asset_sources ADD CONSTRAINT asset_sources_gate_evidence_run_fk`，且最终formal必须有且仅有named `asset_sources_gate_evidence_run_fk` exact four-column `DEFERRABLE INITIALLY DEFERRED` FK；source/reference column order、table/schema与mapping逐字精确，digest/expiry不得入identity。baseline必须没有该FK，formal→baseline→formal必须可逆并重验。missing/partial/duplicate、wrong或quoted alias name、错误columns/order/table/schema、6-column、`NOT DEFERRABLE`/`INITIALLY IMMEDIATE`、dynamic DDL、除该唯一精确top-level创建语句外的任何ALTER、后续任何DROP/VALIDATE/rename/disable lifecycle、额外gate constraint或up/down mismatch必须fail closed。不得缩小PR #153 exact3/trigger/drop auxiliary matrix、现有negative matrix、global exact72/110、owned exact38、ACL/owner/grantor/grantability/C-order/down manifest。恢复开发后的唯一入口是从届时最新`origin/main`创建fresh、独立test-only identity-FK fixture corrective，完成真实RED→GREEN、完整验证、独立复核、PR与合并后才可fresh启动Task19A2a exact12；A2a继续负责pre-up72、up110 direct/effective/owner/grantor/grantability、runtime exact72 edge digest、unexpected111、wrong predecessor ACL、down72 restored、up/down/up、五身份和required双实例dump/restore RED→GREEN，并禁止修改该helper。docs-only exact9B不实现migration/A2a/G2，不改变completion、checkbox或capability；Task19A2c仍必须复用Task28A唯一Worker loop，Task29A若seam不足先做冻结corrective。所有 Provider/Worker在真实 G3/G4前继续`NOT_STARTED/UNAVAILABLE/CLOSED`。
