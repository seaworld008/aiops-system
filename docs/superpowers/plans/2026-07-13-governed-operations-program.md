...
当前唯一执行顺序为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。global routine ACL exact11 已由 PR #152 合并为 `main@d8d632f453929fbe056dc499a6d64144d83011bf`。上一 fresh A2a 因 predecessor72 默认 PUBLIC `EXECUTE` 为两个 capability identity产生合计144条额外 effective edge而停止；PR #152 后的 fresh A2a 又确认 pre-A2a fixture 无条件合成 exact3/trigger/drop，与 formal `000015` 必须内建的同一对象冲突。两次 partial A2a 都是`STOPPED/NOT PASS`，其未提交 worktree/WIP/snapshot永不作为输入。

当前唯一入口是 pre-A2a formal-fixture compatibility exact9 corrective：Phase A 仅修改八份权威文档并先独立复核，Phase B 才唯一修改 test-only `internal/assetcatalog/postgres/migration_corrective_test.go`。fixture 必须在 predecessor“exact3/trigger/drop 全 absent”时合成，在 formal A2a“三者全 exact present”时验证后复用；partial、duplicate、renamed、wrong type/trigger/relation/lifecycle、dynamic DDL或up/down不一致必须fail closed。不得缩小现有negative matrix、global exact72/110、owned exact38、ACL/owner/grantor/grantability/C-order/down manifest。Task19A2a exact12禁止修改该helper，exact9合并后才可从最新`origin/main`fresh启动，继续负责pre-up72、up110 direct/effective/owner/grantor/grantability、runtime exact72 edge digest、unexpected111、wrong predecessor ACL、down72 restored、up/down/up、五身份和required双实例dump/restore RED→GREEN。exact9不实现migration/A2a/G2，不改变completion、checkbox或capability；Task19A2c仍必须复用Task28A唯一Worker loop，Task29A若seam不足先做冻结corrective。所有 Provider/Worker在真实 G3/G4前继续`NOT_STARTED/UNAVAILABLE/CLOSED`。
