...
| Pre-A2a identity-FK fixture compatibility | [Pack 06 exact9B fixture corrective](01-assets/06-source-external-cmdb.md#pre-task-19a2a-identity-fk-fixture-compatibility-exact-9b-corrective) | PR #169 已合并 test-only corrective，状态 `BUILT_CLOSED`；仅修复 inline/top-level exact FK parser、up/down parity 与可逆 fixture extraction，不实现 migration/A2a/G2 | 只接受唯一named exact four-column `DEFERRABLE INITIALLY DEFERRED` FK，baseline无FK且formal→baseline→formal可逆重验；完整负例与 PR #153 auxiliary/global72/110/owned38/ACL/down合同保持强度 |
| Gate evidence schema/domain/admission | [Pack 06 Task 19A2a](01-assets/06-source-external-cmdb.md#task-19a2a-source-gate-schema-domain-and-admission-contract) | merged Task 19A + all pre-A2a contracts + PR #169 test-only fixture corrective → fresh exact12 consumes Pack06 exact72 identity manifest/production digest and proves global110/runtime72、up/down、五身份、required双实例restore plus twelve-table safe receipt/HA digest schema、schema/role admission、pure `GateEvidenceSet/EvidenceVerifier/OutcomeSink` contracts；12 exact files且禁止修改pre-A2a helper，独立真实 G2/PR/merge | 当前`NOT_STARTED/UNAVAILABLE/CLOSED`；无 Repository/API/Worker/assembly；no Provider becomes available |
