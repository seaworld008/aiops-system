...
| `CMDB_CATALOG_V1` | [External CMDB](01-assets/06-source-external-cmdb.md) + [provider-neutral Worker/HA](01-assets/09-discovery-worker-ha-e2e.md) | Task 18A 身份/TLS/固定协议/paging；Task 18B 消费已合并 Worker core/Task 27/PageCommitter，证明增量关系/provenance/lifecycle并产出 neutral descriptor；Task 28B/28C 关闭 same-session recovery/production；Task 19A 装配 Control Plane validation并确保 non-MANUAL publication 为 `PUBLISHED + UNAVAILABLE`；PR #143 reachability docs 与第一次 manager exact-3 contract sync 后，必须先依次 merge capability-identity harness C0、pre-A2a exact-2 与第二次 manager exact-3 evidence sync，fresh Task 19A2a 才独立拥有 qualification schema/domain/admission；post-A2a exact-2 先合并后，Task 19A2b 才拥有 persistence/runtime rechecks/HA durable facts，Task 19A2c 再复用 Task 28A loop装配 API/production且禁止第二 runner；Task 29A 只产可信 HA receipt/真实 binary metrics wiring；Task 19B 唯一拥有 CMDB canary/evaluator/AdmitGate operating proof；Task 29B 只聚合 final matrix | `UNAVAILABLE` |
...
当前 hard gate 由本第一次 manager exact-3 contract sync 持久化：它合并前禁止启动 capability harness、pre-A2a 或 formal A2a；它合并后唯一允许的下一步是从最新 `main` 启动 capability-identity harness C0 exact 8 files。harness 合并后只能推进 pre-A2a exact-2；二者都合并后仍须第二次 manager exact-3 evidence sync，formal A2a 才能从届时最新 `main` fresh 启动。该同步不改 ABI、任务 checkbox 或完成度，不把 fixture、旧 dirty worktree或未合并实现变成事实源；所有 Source/Provider 实际运行能力继续 `NOT_STARTED/UNAVAILABLE/CLOSED`，既有已合并稳定 `Produces` 的 `BUILT_CLOSED` 开发完成度不回退，本 successor 的 G2/G3/G4 继续 deferred。

PR #143 reachability docs corrective 未修改状态页；本第一次 manager exact-3 contract sync 是当前唯一的 `docs/status/current.md` owner，并只把已合并合同与唯一下一入口同步到本矩阵和 Pack 09。后继 capability harness、pre-A2a 与 formal A2a 均不得修改这三个 manager-owned 文件。G3/G4 全部 deferred，不把 `EXTERNAL_CMDB` 或任何 Worker/Provider 标为 `AVAILABLE`。
