...
| `CMDB_CATALOG_V1` | [External CMDB](01-assets/06-source-external-cmdb.md) + [provider-neutral Worker/HA](01-assets/09-discovery-worker-ha-e2e.md) | Task 18A 身份/TLS/固定协议/paging；Task 18B 消费已合并 Worker core/Task 27/PageCommitter，证明增量关系/provenance/lifecycle并产出 neutral descriptor；Task 28B/28C 关闭 same-session recovery/production；Task 19A 装配 Control Plane validation并确保 non-MANUAL publication 为 `PUBLISHED + UNAVAILABLE`；PR #143 reachability docs、第一次 manager exact-3 contract sync、PR #149 capability-identity harness C0、PR #150 pre-A2a exact-2及其manager evidence sync已按序合并，PR #152 exact11已合并为`main@d8d632f453929fbe056dc499a6d64144d83011bf`；其后fresh A2a暴露formal migration与fixture重复exact3/trigger/drop并`STOPPED/NOT PASS`，当前必须先合并pre-A2a formal-fixture compatibility exact9，fresh Task19A2a exact12才可从最新`origin/main`的全新clean worktree消费Pack06唯一exact72 manifest与production digest，独立实现global110/runtime72、up/down、五身份、required双实例restore及qualification schema/domain/admission；post-A2a exact-2 先合并后，Task 19A2b 才拥有 persistence/runtime rechecks/HA durable facts，Task 19A2c 再复用 Task 28A loop装配 API/production且禁止第二 runner；Task 29A 只产可信 HA receipt/真实 binary metrics wiring；Task 19B 唯一拥有 CMDB canary/evaluator/AdmitGate operating proof；Task 29B 只聚合 final matrix | `UNAVAILABLE` |
...
| Pre-A2a formal-fixture compatibility | [Pack 06 exact9 corrective](01-assets/06-source-external-cmdb.md#pre-task-19a2a-formal-fixture-compatibility-exact-9-corrective) | merged PR #152 exact11 → Phase A exact8 docs独立复核 → Phase B exact1 test-only fail-first RED/GREEN/G1/独立复核；baseline exact3/trigger/drop全absent时合成、formal三者全exact present时复用，partial/duplicate/renamed/wrong type/trigger/relation/lifecycle/dynamic DDL/up-down mismatch fail closed | 不改migration/A2a exact12或原negative/global72/110/owned38/ACL/down合同；no completion/G2/G4/capability change |
| Gate evidence schema/domain/admission | [Pack 06 Task 19A2a](01-assets/06-source-external-cmdb.md#task-19a2a-source-gate-schema-domain-and-admission-contract) | merged Task 19A + reachability docs + first manager sync + capability harness + pre-A2a exact-2 + second manager sync + global routine ACL exact11 contract/status sync + merged pre-A2a formal-fixture compatibility exact9 → fresh exact12 consumes Pack06 exact72 identity manifest/production digest and proves global110/runtime72、up/down、五身份、required双实例restore plus twelve-table safe receipt/HA digest schema、schema/role admission、pure `GateEvidenceSet/EvidenceVerifier/OutcomeSink` contracts；12 exact files且禁止修改pre-A2a helper，独立真实 G2/PR/merge | 当前`STOPPED/NOT PASS`；无 Repository/API/Worker/assembly；no Provider becomes available |
