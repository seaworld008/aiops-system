...
[PR #136](https://github.com/seaworld008/aiops-system/pull/136) 已把 Source Gate qualification 唯一持久 ABI 作为精确五文档合同合并：base/parent `cc97545c6a9f9f962cccd0e6ed6c53e94c6e1c18`、reviewed head `56e0bfc0da84682d35ef3e586831944fedad202d`、squash `4ab8e167d6ac61178c81efd81930f2d4ce19f856`，reviewed/squash tree 均为 `d7f3a2d4ad5b3da5b7cd726c2648bf35f1c95b34`，规模 `+171/-33`；exact-head required `go=SUCCESS` 且 [Codex Review](https://github.com/seaworld008/aiops-system/pull/136#issuecomment-5012720578) 未发现重大问题。该事实只冻结 3+23 nullable-column ABI、four-column deferred FK、named deferred closure trigger、15-frame receipt、双 transaction lifecycle 与 test boundary，不表示实现、checkbox、G2/G4 或 `FULL PASS`。其 parent [PR #135](https://github.com/seaworld008/aiops-system/pull/135) 只修正 exact two stale outbox integration test contracts，没有业务能力变化，也不是业务实现基线。

[PR #143](https://github.com/seaworld008/aiops-system/pull/143) 已把 Source Gate reachability 与新 successor 顺序作为精确八文档合同合并：base `ca6bf2cae9c1e187050016a63ac11b0ef020d19b`、final reviewed head `884ffff297dba6367ebbf0111149b6bf5c22a1fa`、squash `38fdcc32786948a900e6482cd1f4ddcca8289701`，reviewed/squash tree 均为 `97406e87b1fb4535e44ee81140835a4f97b82f69`，规模 `+222/-77`；required `go=SUCCESS`，首轮有效 [P1 conditional capability ACL](https://github.com/seaworld008/aiops-system/pull/143#discussion_r3610250222) 已在 final head 修复，exact-head [Codex Review](https://github.com/seaworld008/aiops-system/pull/143#issuecomment-5015214324) 未发现重大问题，merge 后凭证已记录并 resolved。该事实只冻结 exact-36/exact-38 capability reachability、两条 fixed primitive、事务内签名/TTL/replay、exact-8/2/3 ownership 与串行门，不表示 identities、migration、functions、connectors、checkbox 或资格已实现。

[PR #149](https://github.com/seaworld008/aiops-system/pull/149) capability-identity harness 已从 base `641c2e180bf6f2f79384f6d6e639d8be108176b3` 以 reviewed head `36adcb0fac71f518decfff4621e3383205bddc19` squash merge 为 `a5aa76c945108c74c49cbbd56718ff0a9424e635`；reviewed/squash tree 均为 `92e66a15e463aa3081d636299f859311f5405926`，exact-8 `+2008/-89`。PostgreSQL 18.4/TLS 五身份 G2 `2022/2022`、fresh final targeted race `36/36`、fresh G1 与 required `go=SUCCESS` 均通过；[exact-head Codex Review](https://github.com/seaworld008/aiops-system/pull/149#issuecomment-5030643013) 未发现重大问题，完整分页 comments/reviews/reviewThreads 为 `2/0/0`。它只提供隔离 identity/credential/DSN/certificate 与 future-gated ACL harness，不授予函数、不打开 pool 或能力。

[PR #150](https://github.com/seaworld008/aiops-system/pull/150) pre-A2a exact-2 已从 base `8bee1982268dc3ac6c563292c9ce55413e5d9a51` 以 final reviewed head `44e494e9941c6885dd8040b2eec07d064a4d894f` squash merge 为 `main@1a84064eb07453f911b72a38d124abfb8eb7d147`；reviewed/squash tree 均为 `f4085f1c9b551ae8485d82db35fb1c60c66832a7`，exact-2 `+2731/-146`。final targeted normal/race `33/33`、PostgreSQL 18.4/TLS 五身份 G2 `1442/1442`、fresh G1 与 required `go=SUCCESS` 均通过；final-head [Codex Review](https://github.com/seaworld008/aiops-system/pull/150#issuecomment-5032216884) 未发现重大问题。唯一 [P2](https://github.com/seaworld008/aiops-system/pull/150#discussion_r3620766257) 已由 corrective `44e494e` 修复，并在 [merged evidence](https://github.com/seaworld008/aiops-system/pull/150#issuecomment-5035088807) 后 resolved；完整分页 comments/reviews/reviewThreads 为 `4/1/1`，当前 `1 resolved / 0 unresolved`。它只冻结 exact-38 routine/ACL/column/session/down/partial test boundary 与 synthetic structural fixture，不实现 migration/primitive、不打开 pool 或能力。

[PR #153](https://github.com/seaworld008/aiops-system/pull/153) pre-A2a formal-fixture compatibility exact9 已从 parent `d8d632f453929fbe056dc499a6d64144d83011bf` squash merge为 `main@c0b620e6fff9de0b746504f5fb7231fcb4a213c4`，tree为`0af13374448f1386593291631e10a07add41440b`，精确九文件、`+433/-45`。它只冻结 exact3 columns、closure trigger 与 down drop 的baseline/formal双完整状态、可逆复用与auxiliary matrix，不实现migration/A2a或任何能力。其后fresh formal A2a确认Pack06唯一identity FK无论以inline named constraint或唯一精确top-level`ALTER TABLE public.asset_sources ADD CONSTRAINT asset_sources_gate_evidence_run_fk`表达，都会分别被merged extractor/discriminator报为`unreviewed gate-evidence table element`/`unreviewed gate-evidence ALTER`；该partial A2a与停止的`cb00`均为`STOPPED/NOT PASS`且永不作为输入。
...
| `CMDB_CATALOG_V1` | [External CMDB](01-assets/06-source-external-cmdb.md) + [provider-neutral Worker/HA](01-assets/09-discovery-worker-ha-e2e.md) | Task 18A 身份/TLS/固定协议/paging；Task 18B 消费已合并 Worker core/Task 27/PageCommitter，证明增量关系/provenance/lifecycle并产出 neutral descriptor；Task 28B/28C 关闭 same-session recovery/production；Task 19A 装配 Control Plane validation并确保 non-MANUAL publication 为 `PUBLISHED + UNAVAILABLE`；PR #143 reachability docs、第一次 manager exact-3 contract sync、PR #149 capability-identity harness C0、PR #150 pre-A2a exact-2及其manager evidence sync、PR #152 exact11已按序合并，PR #153 formal-fixture exact9已合并为`main@c0b620e6fff9de0b746504f5fb7231fcb4a213c4`、tree`0af13374448f1386593291631e10a07add41440b`；其后fresh formal A2a暴露identity-FK fixture冲突并`STOPPED/NOT PASS`。当前`DEVELOPMENT_PAUSED`；恢复后必须从届时最新`origin/main`新建fresh、独立test-only identity-FK fixture corrective，完成完整RED→GREEN、验证、独立复核与PR合并后，才允许fresh Task19A2a exact12消费Pack06唯一exact72 manifest与production digest，独立实现global110/runtime72、up/down、五身份、required双实例restore及qualification schema/domain/admission；post-A2a exact-2 先合并后，Task 19A2b 才拥有 persistence/runtime rechecks/HA durable facts，Task 19A2c 再复用 Task 28A loop装配 API/production且禁止第二 runner；Task 29A 只产可信 HA receipt/真实 binary metrics wiring；Task 19B 唯一拥有 CMDB canary/evaluator/AdmitGate operating proof；Task 29B 只聚合 final matrix | `UNAVAILABLE` |
...
| Pre-A2a formal-fixture compatibility | [Pack 06 exact9 corrective](01-assets/06-source-external-cmdb.md#pre-task-19a2a-formal-fixture-compatibility-exact-9-corrective) | PR #153已合并；baseline exact3/trigger/drop全absent时合成、formal三者全exact present时复用，partial/duplicate/renamed/wrong type/trigger/relation/lifecycle/dynamic DDL/up-down mismatch fail closed | 不改migration/A2a exact12或原negative/global72/110/owned38/ACL/down合同；no completion/G2/G4/capability change |
| Pre-A2a identity-FK fixture compatibility | [Pack 06 exact9B docs-only contract checkpoint](01-assets/06-source-external-cmdb.md#pre-task-19a2a-identity-fk-fixture-compatibility-exact-9b-corrective) | 截至2026-07-23仅冻结exact8 docs并暂停开发；原Phase B test-only实现`NOT_STARTED`且不属于本交付。恢复后从届时最新`origin/main`新建fresh、独立corrective，完成inline named constraint/唯一精确top-level `ALTER TABLE public.asset_sources ADD CONSTRAINT asset_sources_gate_evidence_run_fk`真实RED→GREEN、完整验证、独立复核、PR/merge | 只接受唯一named exact four-column `DEFERRABLE INITIALLY DEFERRED` FK，baseline无FK且formal→baseline→formal可逆重验；完整负例与PR #153 auxiliary/global72/110/owned38/ACL/down合同不得弱化 |
| Gate evidence schema/domain/admission | [Pack 06 Task 19A2a](01-assets/06-source-external-cmdb.md#task-19a2a-source-gate-schema-domain-and-admission-contract) | merged Task 19A + reachability docs + first manager sync + capability harness + pre-A2a exact-2 + second manager sync + global routine ACL exact11 contract/status sync + merged pre-A2a formal-fixture compatibility exact9 + merged exact9B docs-only checkpoint + resumed fresh merged test-only identity-FK fixture corrective → fresh exact12 consumes Pack06 exact72 identity manifest/production digest and proves global110/runtime72、up/down、五身份、required双实例restore plus twelve-table safe receipt/HA digest schema、schema/role admission、pure `GateEvidenceSet/EvidenceVerifier/OutcomeSink` contracts；12 exact files且禁止修改pre-A2a helper，独立真实 G2/PR/merge | 当前`STOPPED/NOT PASS`；无 Repository/API/Worker/assembly；no Provider becomes available |
| Gate persistence + runtime admission rechecks | [Pack 06 Task 19A2b](01-assets/06-source-external-cmdb.md#task-19a2b-source-gate-persistence-and-runtime-admission-rechecks) | merged Task 19A2a + merged post-A2a exact-2 validation corrective → serializable `RequestQualification/AdmitGate`、server-generated HA Queue/run receipt loader、Audit/Outbox、RequestSync/Queue/PageCommitter expiry/drift rechecks；11 exact files，独立真实 G2/PR/merge | 无 API/Worker/signer；no Provider becomes available |
| Qualification lane/API + generic production assembly | [Pack 06 Task 19A2c](01-assets/06-source-external-cmdb.md#task-19a2c-qualification-lane-api-and-production-assembly) | merged Task 19A2a/19A2b + Task 28A/28B/28C → fixed mTLS API、thin adapter into the one Worker loop、immutable verifier registry + sole outcome sink/signer；23 exact files，独立真实 G2/PR/merge | seam 不足先停并做四个 Task 28A files 的 sequential C0 corrective；禁止第二 runner |
| Provider-neutral two-worker HA/cleanup/restart/response-loss receipt + metrics | [Pack 09 Task 29A](01-assets/09-discovery-worker-ha-e2e.md#task-29a-provider-neutral-two-worker-ha-receipt-and-telemetry) | merged reachability docs + first manager sync + capability harness + pre-A2a exact-2 + second manager sync + global routine ACL exact11 contract/status sync + pre-A2a formal-fixture compatibility exact9 + exact9B docs-only checkpoint + resumed fresh merged test-only identity-FK fixture corrective + fresh merged Task 19A2a + post-A2a exact-2 + merged Task 19A2b/19A2c + Task 28C/28B + opaque external authority binding → verifier over durable facts、sole-signer `TWO_WORKER_HA` receipt and real-binary low-cardinality metrics wiring；12 exact files | 不消费 canary、不写 gate、不产 matrix；production seam 不足先停做六文件 corrective |
...
| Signed final Provider matrix + final E2E/CI | [Pack 09 Task 29B](01-assets/09-discovery-worker-ha-e2e.md#task-29b-signed-provider-matrix-and-final-e2e-ci) | only after the unique successor sequence has reached merged Task 19B: all Provider-owned current per-source gate/canary + Task 29A HA receipts → signed exact-row acceptance matrix and final UI/CI aggregation | 不重建 HA/canary/gate；G4/release |

Task 18B 不拥有 Queue、Worker common state machine、PageCommitter SQL、production registry 或 binary；Task 28A 不 import/注册 Provider；Task 28B 不修改 core/Provider；Task 28C 不重建 neutral/provider descriptor并唯一冻结 production observer/decorator seam。Task 19A 的 Control Plane 只 import `internal/sourceprofile` metadata/admission，不得 import External CMDB Provider/Provider HTTP-client/credential-session graph，也不得拥有 endpoint、credential 或 runtime material；既有 Control Plane HTTP server不在此禁令内。Source Gate successor 的唯一顺序逐字为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → pre-A2a identity-FK fixture compatibility exact-9B docs-only contract checkpoint → [DEVELOPMENT_PAUSED] → resumed fresh test-only identity-FK fixture corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。exact9B仅冻结exact8 docs；恢复后的fresh test-only corrective与Task19A2a exact12零重叠，A2a禁止修改pre-A2a helper。每个实现 Batch 新窗口独立真实 G2/PR/merge且 a/b/c files 两两零重叠。Task 19A2a 唯一拥有 schema/domain/admission，Task 19A2b 唯一拥有 gate persistence/runtime rechecks，Task 19A2c 唯一拥有 generic production assembly并只能复用 Task 28A `worker.go/worker_test.go/claim_runtime.go/claim_runtime_test.go` loop；seam 不足时先停并用仅这四文件的 sequential C0 corrective，禁止第二 runner。Task 29A exact files 与 Task 19B/29B 零重叠，`ha.go` 只验证 Repository-loaded facts并经 19A2c sole signer封存，metrics 必须接真实 binary；production seam 不足时先停做冻结的六文件 corrective。Task 19B 唯一拥有 External CMDB canary/evaluator/decision并只通过 `AdmitGate` 开门；Task 29B 只读聚合，不是 admission input。所有后继只消费已合并 `Produces`。External CMDB successor G2 在 harness 合并后必须使用 PostgreSQL 18.4 TLS 和 control/migration/application/sealer/admitter 五 DSN，缺环境或 Skip 不得算 PASS；两 Worker/production recovery/HA/restart 未跑前旧 Task 18 不可勾完，能力继续 `NOT_STARTED/UNAVAILABLE/CLOSED`。

[PR #153](https://github.com/seaworld008/aiops-system/pull/153) exact9 已从 parent `d8d632f453929fbe056dc499a6d64144d83011bf` squash merge为 `main@c0b620e6fff9de0b746504f5fb7231fcb4a213c4`、tree`0af13374448f1386593291631e10a07add41440b`。其后fresh A2a确认合法identity FK在top-level/inline两种表达下都被merged fixture拒绝并保持`STOPPED/NOT PASS`；停止的`cb00`与其他dirty/stopped A2a worktree/WIP/snapshot永不作为输入，任何未完成实现都不提交、不合并。

截至2026-07-23用户决定暂停开发；当前 hard gate 只把 pre-A2a identity-FK fixture compatibility exact9B 的 exact8 权威文档冻结为docs-only暂停检查点，原Phase B test-only实现永久停止于本轮并保持`NOT_STARTED`，不是本检查点或其后文档PR的交付。冻结的未来实现合同要求inline named constraint和唯一精确top-level`ALTER TABLE public.asset_sources ADD CONSTRAINT asset_sources_gate_evidence_run_fk`分别以已-formal up/down产生真实RED；最终formal必须有且仅有named `asset_sources_gate_evidence_run_fk` exact four-column `DEFERRABLE INITIALLY DEFERRED` FK，baseline无FK且formal→baseline→formal可逆重验。missing/partial/duplicate、wrong/quoted alias name、错误columns/order/table/schema、6-column、digest/expiry identity、wrong deferral、dynamic DDL、除该唯一精确top-level创建语句外的任何ALTER、后续任何DROP/VALIDATE/rename/disable lifecycle、额外gate constraint或up/down mismatch全部fail closed。未来corrective不得削弱PR #153 exact3/trigger/drop auxiliary或现有adversarial/global exact72/110/owned exact38/ACL/owner/grantor/grantability/C-order/down manifest，不实现migration/A2a/G2，也不改变完成度、checkbox或能力。恢复开发后的唯一入口是从届时最新`origin/main`创建fresh、独立test-only identity-FK fixture corrective并完成RED→GREEN、完整验证、独立复核、PR与合并；之后才允许fresh Task19A2a exact12，formal A2a仍不得修改该helper。docs-only exact9B不等于corrective已实现。所有Source/Provider实际运行能力继续`NOT_STARTED/UNAVAILABLE/CLOSED`，既有已合并稳定`Produces`的`BUILT_CLOSED`开发完成度不回退，本successor的G2/G3/G4继续deferred。
