...
- 仅 exact `PUBLISHED` revision 且 source `ACTIVE + AVAILABLE`（需要真实资格时还须 current unexpired gate evidence）可创建 ordinary Discovery/Import/Ingestion Run 或执行 `RequestSync`；修订、引用、作用域、身份、checkpoint、evidence 或 gate 漂移立即停止。后继 Task 19A2a schema/domain + Task 19A2b persistence + Task 19A2c workload lane 的 `QUALIFICATION` 是唯一 publication-closed 例外，不属于本包普通同步/API，也不得投影 Catalog。
...
#### CSV Profile Registry admission corrective（2026-07-17）

Task 15A 的 parser 合并后，Task 13 Registry 新增显式、可信安装 selector `csv-rfc4180-v1`。该 selector 只在服务端配置中绑定 exact `CSV_IMPORT / CSV_RFC4180_V1 / CSV_RFC4180_V1` Profile，仍不持久化。Profile 构造器唯一输入是服务端给出的非空、符合 opaque reference grammar 的 `CredentialReferenceID`；用途固定为 `IMPORT_SIGNATURE_VERIFY`。caller 不能提供或覆盖该 reference。扩展 Registry constructor 必须先 bootstrap exact `manual-v1 / ManualProfileV1()`，再追加 registrations；因此 CSV-only registration 产生 MANUAL+CSV Registry，而不是替换基础 Profile。默认 `NewBuiltinSourceProfileRegistry()` 继续只安装 `manual-v1`；CSV 只能通过不可变 Registry constructor 显式追加。reserved identity 固定双向映射 `manual-v1 ↔ MANUAL_V1`、`csv-rfc4180-v1 ↔ CSV_RFC4180_V1`，且 CSV registration 必须与 `CSVProfileV1(同一 CredentialReferenceID)` 的所有标量、列表、canonical bytes 与 SHA exact 相等；该合法 opaque reference 值是唯一配置变量。selector 与 `ProfileCode` 还分别唯一；duplicate、alias、same-code semantic/reference drift 或无效 reference 都在 Repository 构造/写入前 fail closed。

Resolved CSV Profile 固定为 `ON_DEMAND + OBJECT_SEQUENCE + EXPLICIT_ITEM_ENVIRONMENT`；Integration/Trust/Network/Schedule 均为 `NONE`；Parser/Compatibility=`CSV_RFC4180_V1`、DLP=`ASSET_SAFE_V1`；rate/window=`1/1`、backpressure=`1/60`；max items/relations=`2000/2000`、max page/document bytes=`33554432/33554432`；typed extension 为 `null`。trusted path codes 与当前九个 closed relationship types 都按 C locale 排序。canonical manifest bytes（1100 bytes）及 SHA-256 固定为：

```json
{"backpressure_base_seconds":1,"backpressure_max_seconds":60,"compatibility_class":"CSV_RFC4180_V1","credential_purpose":"IMPORT_SIGNATURE_VERIFY","dlp_policy_code":"ASSET_SAFE_V1","environment_mapping_mode":"EXPLICIT_ITEM_ENVIRONMENT","freshness_kind":"OBJECT_SEQUENCE","integration_mode":"NONE","max_document_bytes":33554432,"max_page_bytes":33554432,"max_page_items":2000,"max_page_relations":2000,"network_mode":"NONE","parser_code":"CSV_RFC4180_V1","profile_code":"CSV_RFC4180_V1","provider_kind":"CSV_RFC4180_V1","rate_limit_requests":1,"rate_limit_window_seconds":1,"relationship_types":["CONTAINS","DELIVERED_BY","DEPENDS_ON","LOGS_TO","MANAGED_BY","MONITORED_BY","PRIMARY_RUNTIME_FOR","RUNS_ON","TRACES_TO"],"schedule_mode":"NONE","source_kind":"CSV_IMPORT","sync_mode":"ON_DEMAND","trust_mode":"NONE","trusted_path_codes":["CSV_V1_DISPLAY_NAME_COLUMN","CSV_V1_ENVIRONMENT_ID_COLUMN","CSV_V1_EXTERNAL_ID_COLUMN","CSV_V1_KIND_COLUMN","CSV_V1_PROVIDER_KIND_COLUMN","CSV_V1_RELATION_COLUMNS","CSV_V1_TYPE_DETAILS_EMPTY"],"typed_extension_code":null,"version":"asset-source-profile-manifest.v1"}
```

`ProfileManifestSHA256=9a9739783fbb84a66653271202e06e2e6b2cdcffc268564151d0c6035cbf4941`。canonical provider schema bytes 固定为 `{"additionalProperties":false,"properties":{},"type":"object"}`（62 bytes），`CanonicalProviderSchemaSHA256=99334726611ccf58a148b0814696bfa6fe08c1b2d027e946beccf5a74331c9aa`。Repository 的 `CreateSource`、`CreateRevision` 与 `SourceProfileAdmissionResolver` 必须消费同一注入 Registry；任一 nil/unknown/drift 都拒绝，错误不得回显 Credential Reference。两个 create 路径在写入前共同对 exact Tenant/Workspace + `ProfileCode` 建立 serializable predicate，重载该 Scope 全部已有同 code revision，将 canonical manifest/schema+SHA、Integration/Credential/Trust/Network、rate/backpressure、schedule/extension 与当前 Registry exact 比较；服务重启、第二 Source 或 successor 配置变化造成的 same-code reference/semantic drift 必须零写入回滚，并发首写只能有一种语义获胜，只有新的 selector 加新的 versioned `ProfileCode` 才能改变语义。事务还必须先读取已消费的 idempotency Audit/command hash：changed selector（包括合法但未安装的 selector）replay 固定返回 `ErrIdempotency`，matching replay 或 fresh mutation 才解析 Profile；fresh unknown selector 仍返回 `ErrNotFound`。该顺序修正不扩展 Task 15B。该 corrective 只允许创建 `ACTIVE + UNAVAILABLE` Source 与 `DRAFT` revision，CSV Adapter/Importer、签名校验、quarantine、Worker、HTTP、backpressure/cleanup、真实 Credential 和 Provider gate 仍为 `NOT_STARTED / UNAVAILABLE / CLOSED`。
...
当前实现中 `internal/assetcatalog/postgres/source_revisions.go` 的 broad Publish mutation 仍会把所有 profile 直接写成 `AVAILABLE`，与上述合同冲突。按冻结 DAG，Task 19A 是该文件下一位唯一 owner：它必须保持 `MANUAL_V1` 既有特例，同时把每个需真实资格的 non-MANUAL publication 原子写为 `PUBLISHED + UNAVAILABLE`；Task 19A2a 冻结 qualification schema/domain，Task 19A2b 才新增 persistence/runtime rechecks 与唯一 `AdmitGate`，Task 19A2c 再装配 workload-only lane/sole sink+signer。四者依次独立合并前任何 non-MANUAL profile 都不得借当前 Publish 行为进入生产 registry/gate。
...
Task 19A2c 后续只可消费已合并 Task 19A2a schema/domain 与 Task 19A2b persistence，在同一 OpenAPI 中新增 fixed mTLS workload qualification operation，并由独立 production qualification listener/identity 调用；它不是上述 OIDC browser route、不得加入 `SourceActionAdmission`/`effective_actions`，且请求只能携带 path/CAS/idempotency safe facts，opaque lab binding 必须由服务端装配。普通 `:sync`、imports 和 ingestion-batches 不得被重解释为 qualification。

This task is the sole owner of Source mutation routes deferred by Packs 03–04. Public `CreateSource` accepts only a server-returned `source_profile_id` selector, name and allowed Environment authority IDs; public `CreateRevision` accepts only that selector and authority plus bounded reason and current Source CAS/`If-Match` facts. `CredentialReferenceID`、`TrustReferenceID`、`NetworkPolicyReferenceID`、`IntegrationID`、`SyncMode`、`Schedule`、Profile/Freshness/rate/page/backpressure limits、typed-extension code/digest、ProviderKind/ProfileCode/canonical manifest/schema and every other resolved fact are owned exclusively by the installed `SourceProfileRegistry`；the caller cannot submit or override them. The Repository resolves that immutable Profile before creating stable Source plus revision 1 or a later immutable revision in one `SERIALIZABLE` transaction. No reduced ProviderKind/Integration form、caller-resolved-fact form or stable-Source-only insert is permitted.
...
- CSV header is fixed at these 11 columns: `environment_id,provider_kind,external_id,kind,display_name,object_version,deleted,tombstone_reason,relation_type,relation_target_environment_id,relation_target_external_id`. `object_version` is the CSV Adapter's positive decimal record version and is never mapped to persisted `source_revision`; the server derives the exact Source definition revision from the published Run. A relation row must supply both target fields；its source `environment_id` and `relation_target_environment_id` must be the same Environment and both must belong to the exact published revision's immutable authority allow-list. `CSV_RFC4180_V1` rejects every cross-Environment relation with `CSV_RELATION_INVALID`. Cross-Environment CSV relations require a future versioned schema/header that explicitly carries an opaque `CrossEnvironmentPolicyReferenceID`；no such V2 is implemented, and the current 11-column header must not be extended or reinterpreted. M1E only compares the locked resolver's expected policy reference exactly；the V1 parser must never inject or invent one. A row without a relation must leave both target fields empty.

Task 15A delivers only the bounded `CSV_RFC4180_V1` parser and its parser/checkpoint intermediate. Detached-signature verification、quarantine storage、the common sealed checkpoint codec、fenced Run/Reconciler integration、HTTP upload、durable backpressure and terminal cleanup remain Task 15B and are `NOT_STARTED / UNAVAILABLE / CLOSED`；Task 15A evidence cannot be used to claim an import route, runnable Source, Provider availability or completed Task 15.
