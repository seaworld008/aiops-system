...
M1D Checkpoint Codec + M1E PageCommitter + Task 27 Queue/Cleanup/Limiter
  ...
  -> External CMDB Task 19A
  -> reachability docs corrective → manager exact-3 contract sync
  -> source-gate capability-identity harness C0
  -> pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync
  ...
  -> fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c
...
后继 Batch 只能从最新 `origin/main` 读取已合并 `Produces`。Task 18B 不得读取 Task 28A 未提交文件；Task 28B 不得读取 Task 18B 未合并 descriptor；Task 28C 不得读取 Task 28B 或其他 Provider 未合并实现。已合并 Task 28C/19A 后，Source Gate 后半段对 External CMDB 严格为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → pre-A2a identity-FK fixture corrective (PR #169 merged) → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。PR #149 capability harness、PR #150 pre-A2a exact-2及其manager evidence sync、PR #152 exact11与PR #153 exact9已按序合并；PR #153基线为`main@c0b620e6fff9de0b746504f5fb7231fcb4a213c4`、tree`0af13374448f1386593291631e10a07add41440b`。其后fresh formal A2a暴露合法top-level identity FK报`unreviewed gate-evidence ALTER`、合法inline named FK报`unreviewed gate-evidence table element`并保持`STOPPED/NOT PASS`。PR #169 已合并 exact9B 的 fresh test-only fixture corrective；该 Batch 仅为 `BUILT_CLOSED` 测试边界，解除了 fixture parser blocker 但不实现 A2a/G2。冻结合同只允许唯一named `asset_sources_gate_evidence_run_fk` exact four-column `DEFERRABLE INITIALLY DEFERRED` FK的inline named constraint或唯一精确top-level `ALTER TABLE public.asset_sources ADD CONSTRAINT asset_sources_gate_evidence_run_fk`两种表达，baseline必须无FK且formal→baseline→formal可逆重验；missing/partial/duplicate、alias/name、columns/order/table/schema、6-column、digest/expiry identity、wrong deferral、dynamic DDL、除该唯一精确top-level创建语句外的任何ALTER、后续DROP/VALIDATE/rename/disable lifecycle、extra/up-down mismatch全部fail closed，且不得收窄PR #153 exact3/trigger/drop auxiliary、global exact72/110、owned exact38、ACL/owner/grantor/grantability/C-order/down manifest或现有negative matrix。恢复后的唯一入口是从届时最新`origin/main`创建fresh、独立test-only identity-FK fixture corrective并完成RED→GREEN、完整验证、独立复核、PR/merge；之后才可fresh A2a。A2a exact12禁止修改该pre-A2a helper，停止的`cb00`与其他dirty/stopped A2a worktree/WIP/snapshot永不作为输入，任何未完成实现不提交、不合并。PR #169 test-only corrective 不实现 migration/A2a/G2、不提升 Provider/Worker/Capability。vSphere 是显式 closed 分支：Task 21A 不是 Task 28C 可消费的 durable descriptor；Task 21B0/21B 未真实完成前，Task 28C 必须不注册 vSphere，Task 22/29A/29B 也不得合成 vSphere availability evidence。Task 28C 可为其他已闭合 Provider 先合并；若其先合并，后续 vSphere registry extension 必须另行冻结不重叠 exact files，不能借 Task 21A 自动出现。

[PR #149](https://github.com/seaworld008/aiops-system/pull/149) capability-identity harness 从 base `641c2e180bf6f2f79384f6d6e639d8be108176b3`、reviewed head `36adcb0fac71f518decfff4621e3383205bddc19` squash merge 为 `a5aa76c945108c74c49cbbd56718ff0a9424e635`；reviewed/squash tree 均为 `92e66a15e463aa3081d636299f859311f5405926`，exact-8 `+2008/-89`。PostgreSQL 18.4/TLS 五身份 G2 `2022/2022`、fresh final targeted race `36/36`、fresh G1 与 required `go=SUCCESS` 均通过；[exact-head Codex Review](https://github.com/seaworld008/aiops-system/pull/149#issuecomment-5030643013) 未发现重大问题，完整分页 comments/reviews/reviewThreads 为 `2/0/0`。该 Batch 只提供隔离 identity/credential/DSN/certificate 与 future-gated ACL harness，不授予函数、不打开 pool 或能力。

[PR #150](https://github.com/seaworld008/aiops-system/pull/150) pre-A2a exact-2 从 base `8bee1982268dc3ac6c563292c9ce55413e5d9a51`、final reviewed head `44e494e9941c6885dd8040b2eec07d064a4d894f` squash merge 为 `main@1a84064eb07453f911b72a38d124abfb8eb7d147`；reviewed/squash tree 均为 `f4085f1c9b551ae8485d82db35fb1c60c66832a7`，exact-2 `+2731/-146`。final targeted normal/race `33/33`、PostgreSQL 18.4/TLS 五身份 G2 `1442/1442`、fresh G1 与 required `go=SUCCESS` 均通过；final-head [Codex Review](https://github.com/seaworld008/aiops-system/pull/150#issuecomment-5032216884) 未发现重大问题。唯一 [P2](https://github.com/seaworld008/aiops-system/pull/150#discussion_r3620766257) 已由 corrective `44e494e` 修复，并在 [merged evidence](https://github.com/seaworld008/aiops-system/pull/150#issuecomment-5035088807) 后 resolved；完整分页 comments/reviews/reviewThreads 为 `4/1/1`，当前 `1 resolved / 0 unresolved`。该 test-only Batch 只冻结 exact-38 routine/ACL/column/session/down/partial test boundary 与 synthetic structural fixture，不实现 migration/primitive、不打开 pool 或能力。

pre-A2a formal-fixture compatibility exact9已由PR #153合并；pre-A2a identity-FK fixture compatibility exact9B的未来实现合同由[Pack 06对应小节](./06-source-external-cmdb.md#pre-task-19a2a-identity-fk-fixture-compatibility-exact-9b-corrective)唯一冻结。截至2026-07-23该exact9B只是exact8 docs-only暂停检查点，Phase B test-only实现`NOT_STARTED`且不是本交付。恢复后须从届时最新`origin/main`创建fresh、独立corrective，以inline named constraint/唯一精确top-level `ALTER TABLE public.asset_sources ADD CONSTRAINT asset_sources_gate_evidence_run_fk`已-formal up/down真实RED完成严格parser/state-aware可逆baseline extraction、完整验证、独立复核、PR/merge；其文件与Task19A2a exact12零重叠，后者不得修改`internal/assetcatalog/postgres/migration_corrective_test.go`。该后继实现合并前，本Pack的Task29A/29B和任何fresh A2a均不得启动。

每个 Batch 都由新窗口独立真实 G2/PR/merge，Task 29A 不消费 canary、不写 gate，Task 19B 不拥有 HA runner/metrics，Task 29B 不重建 gate/canary/HA。源文件核验发现 process-local Broker 不能满足 replacement Worker cleanup 后，原来的“Task 28A core + Task 28B production”最小拆分增加一个独立 transport Batch，并把 production 顺延为 Task 28C；这仍未解决 vSphere PropertyCollector continuity。后续因 canary↔AVAILABLE、Task 19B↔旧 Task 29 证据循环、46-file context risk 与 23-file persistence L Batch，最终拆成 12-file Task 19A2a、11-file Task 19A2b、23-file Task 19A2c、Task 29A 与 Task 29B，三段 Source Gate files 零重叠。
...
**Interfaces:**
- Consumes only the latest-`main` merged predecessor prefix admitted by `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → pre-A2a identity-FK fixture corrective (PR #169 merged) → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`：Task 29A 只能在该链已依次合并到 Task 19A2c 后开始；这是唯一 hard admission，前文任何缩写都不能绕过 exact-8 harness、两次 exact-2、历史两次 manager sync、exact11原子门、exact9 fixture门、exact9B docs checkpoint或恢复后的test-only identity-FK fixture门。其直接稳定输入为 Task 19A2a safe receipt/HA schema、Task 19A2b server-generated durable fact loader、Task 19A2c fixed `TWO_WORKER_HA` qualification lane/immutable verifier registry/sole sink+signer、Task 28C real binary/registry/observer decorators、Task 28B fixed mTLS recovery client/same-attempt composition，以及通过 opaque lab binding 预置的真实外部 shared session authority。它不拥有 authority server/binary，脚本不得启动或用 fake/shared process map 代替它。vSphere row 还必须消费 Task 21B0 的真实 resident PropertyCollector authority与 Task 21B continuity evidence；Task 28B cleanup recovery单独存在时 vSphere HA receipt必须拒绝。
...
**Interfaces:**
- Consumes only after the latest-`main` merged successor chain `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → pre-A2a identity-FK fixture corrective (PR #169 merged) → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B` has reached Task 19B；this is Task 29B's sole hard-admission and any earlier abbreviated ownership/DAG text cannot bypass it。It then consumes only already-completed signed safe facts for each exact Provider row：current validation、protocol/negative/DLP/provenance/checkpoint/delete-recovery/rate/backpressure/cleanup、Task 29A `TWO_WORKER_HA` receipt、Provider-owned non-production canary receipt、current atomic gate-evidence pointer and status。It does not call Provider、queue qualification、kill Worker、re-run HA/canary or write a gate。
