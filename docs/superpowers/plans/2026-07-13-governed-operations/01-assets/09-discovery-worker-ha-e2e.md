...
pre-A2a formal-fixture compatibility exact9已由PR #153合并；pre-A2a identity-FK fixture compatibility exact9B的未来实现合同由[Pack 06对应小节](./06-source-external-cmdb.md#pre-task-19a2a-identity-fk-fixture-compatibility-exact-9b-corrective)唯一冻结。PR #169 已合并该 exact9B fresh test-only corrective；它完成 strict parser/state-aware 可逆 extraction 与负例验证，不修改 migration、Task19A2a exact12 或运行能力。Task29A/29B 仍须等待后续 Task19A2a/19A2b/19A2c 链。
