...
后继 Batch 只能从最新 `origin/main` 读取已合并 `Produces`。Task 18B 不得读取 Task 28A 未提交文件；Task 28B 不得读取 Task 18B 未合并 descriptor；Task 28C 不得读取 Task 28B 或其他 Provider 未合并实现。已合并 Task 28C/19A 后，Source Gate 后半段对 External CMDB 严格为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。PR #149 capability harness、PR #150 pre-A2a exact-2及其manager evidence sync已按序合并；PR #152 exact11 已合并到 `main@d8d632f453929fbe056dc499a6d64144d83011bf`。其后 fresh A2a 暴露 pre-A2a fixture 与 formal `000015` 重复 exact3/trigger/drop 的新 blocker并保持`STOPPED/NOT PASS`；当前唯一入口是 exact9，合并后才可从最新`origin/main`全新创建Task19A2a exact12。exact9只允许baseline三对象全absent时合成、formal三对象全exact present时验证复用；partial/duplicate/renamed/wrong type/trigger/relation/lifecycle/dynamic DDL/up-down mismatch全部fail closed，且不得收窄global exact72/110、owned exact38、ACL/owner/grantor/grantability/C-order/down manifest或现有negative matrix。A2a exact12禁止修改该pre-A2a helper，停止的partial A2a/WIP/snapshot永不作为输入。exact9不实现migration/A2a/G2、不提升completion或capability。vSphere 是显式 closed 分支：Task 21A 不是 Task 28C 可消费的 durable descriptor；Task 21B0/21B 未真实完成前，Task 28C 必须不注册 vSphere，Task 22/29A/29B 也不得合成 vSphere availability evidence。Task 28C 可为其他已闭合 Provider 先合并；若其先合并，后续 vSphere registry extension 必须另行冻结不重叠 exact files，不能借 Task 21A 自动出现。
...
pre-A2a formal-fixture compatibility exact9 的完整合同由 [Pack 06 对应小节](./06-source-external-cmdb.md#pre-task-19a2a-formal-fixture-compatibility-exact-9-corrective) 唯一冻结：Phase A exact8 权威文档先独立复核，Phase B exact1 test-only fixture再以已-formal up/down真实RED完成state-aware/idempotent兼容。其 exact9 文件与 Task19A2a exact12 零重叠；后者不得修改 `internal/assetcatalog/postgres/migration_corrective_test.go`。exact9合并前，本Pack的Task29A/29B和任何fresh A2a均不得启动。
...
**Interfaces:**
- Consumes only the latest-`main` merged predecessor prefix admitted by `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`：Task 29A 只能在该链已依次合并到 Task 19A2c 后开始；这是唯一 hard admission，前文任何缩写都不能绕过 exact-8 harness、两次 exact-2、历史两次 manager sync、exact11原子门或exact9 fixture门。其直接稳定输入为 Task 19A2a safe receipt/HA schema、Task 19A2b server-generated durable fact loader、Task 19A2c fixed `TWO_WORKER_HA` qualification lane/immutable verifier registry/sole sink+signer、Task 28C real binary/registry/observer decorators、Task 28B fixed mTLS recovery client/same-attempt composition，以及通过 opaque lab binding 预置的真实外部 shared session authority。它不拥有 authority server/binary，脚本不得启动或用 fake/shared process map 代替它。vSphere row 还必须消费 Task 21B0 的真实 resident PropertyCollector authority与 Task 21B continuity evidence；Task 28B cleanup recovery单独存在时 vSphere HA receipt必须拒绝。
