...
The corrected migration owns the previous unordered 35 routine identities plus exactly `public.asset_catalog_lock_exact_service_binding(uuid,uuid,uuid,uuid)`；there is no overload or other 37th routine。The lock is `RETURNS boolean LANGUAGE plpgsql VOLATILE STRICT PARALLEL UNSAFE SECURITY DEFINER`，owned by `aiops_schema_owner`，fixed `search_path=pg_catalog, public, pg_temp`，requires `transaction_isolation='serializable'` and `transaction_read_only='off'`，then locks exact Service `FOR KEY SHARE` before exact legacy binding `FOR SHARE` and requires `mapping_status='EXACT'`。The reviewed trigger manifest is the existing 39 plus bucket mutation/delete/truncate and permit immutable/delete/truncate，exactly 45。Down takes the exact 20-relation set（six prerequisites + Audit + Outbox + twelve owned）in one `ACCESS EXCLUSIVE ... NOWAIT` statement，drops the existing five cycle FKs，then child-first `service_asset_bindings→asset_relationships→asset_conflicts→asset_type_details→assets→asset_observations→asset_source_limit_permits→asset_source_limit_buckets→asset_source_runs→asset_source_revision_authorities→asset_source_revisions→asset_sources` before dependent typed helpers；no `CASCADE`，and any member conflict is `55P03` before change。
