...
Add production `schema_admission.go` and `database_role_admission.go` probes consumed later by Control Plane assembly. Their constructors require the explicit trusted schema name (`public` in production); they never use `current_schema`, `search_path` object resolution or a caller-selected schema. Task 1 owns four exact base roles；all four are `NOSUPERUSER/NOCREATEDB/NOCREATEROLE/NOREPLICATION/NOBYPASSRLS`：`aiops_migrator` is `LOGIN/NOINHERIT`，`aiops_schema_owner` and `aiops_control_plane_runtime` are `NOLOGIN/NOINHERIT`，and `aiops_control_plane_workload` is `LOGIN/INHERIT`。The only base membership edges are migrator→schema owner with `inherit=false,set=true,admin=false` and workload→runtime with `inherit=true,set=false,admin=false`；neither runtime identity has any edge to migrator/schema owner。Source Gate A2a separately consumes two preprovisioned capability LOGIN identities—`aiops_source_gate_sealer` and `aiops_source_gate_admitter`—both `NOINHERIT` with all five dangerous flags disabled、no membership and mutually distinct credentials/DSNs；they are neither base ACL carriers nor extension owners。Environment IaC/bootstrap precreates the two roles and credentials before A2a，but exact-36 application database/schema ACL explicitly excludes both capability identities：the database owner is the NOLOGIN schema owner，PUBLIC has no `CONNECT|TEMP`，only migrator/workload have direct `CONNECT`，and direct schema ACL is only owner `CREATE+USAGE` plus runtime `USAGE`。Only after exact-38 schema/routine postflight may the future-gated helper grant sealer/admitter direct database `CONNECT` and schema `USAGE` as owner，then require full role/capability admission before exposing connectors；exact-36、down、unknown or partial state revokes and proves these ACL absent，with unknown/partial then closed。Migration、application、seal and admit DSNs remain four distinct identities even while the two capability application connections are intentionally unusable pre-A2a。The pre-`000015` `000012_outbox_event_routing.up.sql`/`.down.sql` pair remains the sole transaction-prohibited exception and retains its exact fresh-connection/OID/definition/cleanup contract。Migration connection admission requires `session_user=current_user='aiops_migrator'` before `BEGIN`，then `SET LOCAL ROLE aiops_schema_owner` for reviewed DDL/grants and `RESET ROLE` before `COMMIT`；ordinary application admission requires exact workload identity，while post-A2a two fixed Source Gate admission probes require their exact session identities and reject role switching。Runtime has `USAGE` on `public`，read-only `SELECT` on `workspaces/environments/services/service_bindings`（no direct ACL on `tenants/integrations` and no parent-table UPDATE/grant option），and reviewed relation/column privileges on the twelve Asset relations。After Source Gate A2a，Sources/Runs have no relation-level INSERT/UPDATE：Source column grants exclude the three pointer columns；Run INSERT includes legacy initial columns plus exact 7 immutable queue-binding columns，Run UPDATE includes only legacy lifecycle columns，the other 16 protected INSERT columns and all 23 protected UPDATE columns are absent。Other existing Asset relation grants、bucket column-only `UPDATE(next_token_at,last_receipt_id,version,updated_at)` and no permit UPDATE remain unchanged。Direct runtime/workload `FOR KEY SHARE|FOR SHARE|FOR UPDATE` on `services/service_bindings` is `42501`；the only runtime SECURITY DEFINER capability remains exact EXECUTE on `asset_catalog_lock_exact_service_binding(uuid,uuid,uuid,uuid)`。Audit/Outbox retain their exact reviewed column ACL。The postflight/down、schema/role admission、dump/restore and negative tests track initial/up/down/up/partial semantics。All Asset/Audit/Outbox/Limiter `DELETE/TRUNCATE` and every other column privilege are absent。After A2a all 38 functions revoke PUBLIC EXECUTE；runtime receives the exact 17 pure non-trigger signatures plus the existing parent-lock entry，sealer receives only receipt-seal EXECUTE，admitter receives only gate-admit EXECUTE，the 18 trigger-returning routines remain OWNER-only，and neither capability identity has relation/sequence privileges。The initially empty extension-owner ABI remains unchanged；the sole bootstrap runbook separately owns these non-owner capability identities and the conditional ACL reconciliation。
The exact-36 direct schema ACL is `OWNER=CREATE+USAGE` and `RUNTIME=USAGE`；the exact-38 postflight/grant state adds only `SEALER=USAGE` and `ADMITTER=USAGE`。Likewise exact-36 direct database ACL is owner default plus migrator/workload `CONNECT`，while exact-38 adds only sealer/admitter `CONNECT`；all semantic grantors are fixed to OWNER，and no LOGIN except owner receives database/schema `CREATE` or `TEMP`。`PUBLIC/MIGRATOR/WORKLOAD` have no direct schema privilege and workload receives `USAGE` only by inheriting runtime。Admission normalizes `COALESCE(nspacl,pg_catalog.acldefault('n',nspowner))` and accepts only the row set for the admitted exact-36 or exact-38 manifest；default PUBLIC `USAGE`、wrong-version capability ACL、unknown/unexpected grantee or grantor、duplicate or extra ACL all close admission。`DatabaseRoleAdmission.Check` is solely the application-startup admission and fixes `session_user=current_user='aiops_control_plane_workload'`，including exact workload→runtime `inherit=true,set=false,admin=false`；migration identity/role-switch and post-A2a capability identity admission run as separate fixed runner/harness probes and are never caller-selected modes。
...
Recovery test uses `recovery_container_test.go` to start two distinct digest-pinned PostgreSQL 18.4 instances，prove different `system_identifier` values/data checksums，and preprovision the exact four base-role graph plus the two ACL-empty capability identities while role OIDs differ。Both databases start with exact-36 database/schema ACL；CI applies `000001..000015` from the migration DSN only after `session_user=current_user=aiops_migrator`，using the SET-only edge so every user relation/function—including Audit/Outbox and predecessor parents—is owned by `aiops_schema_owner`。Only after exact-38 postflight does the helper grant capability CONNECT/USAGE and require full admission；before dump，catalog plus `pg_restore --list` must prove the allowed archive owner set is exactly the schema owner。Source uses `pg_dump --format=custom --role=aiops_schema_owner`；the clean precreated target remains at exact-36 ACL and restore first proves migrator session identity，then runs non-superuser `pg_restore --single-transaction --role=aiops_schema_owner` so archive schema/relation/function owner and ACL records are preserved。After restored exact-38 postflight，the same helper reconciles target capability ACL before full schema/role/capability admission；down/partial restores revoke-and-absent。`--no-owner`、`--no-acl`、`--disable-triggers`、`--use-set-session-authorization`、superuser restore and ownership rewriting are forbidden；database-level ACL is verified from the conditional preprovision/reconciliation fixture，not falsely attributed to a non-`--create` archive。Representative twelve-table plus scoped Audit/Outbox data is restored，checksummed，then all required admission probes must pass before FK/digest/lifecycle/immutability/Limiter receipt/parent-lock ACL assertions；wrong role flag/membership、database/schema owner/ACL、object owner/ACL or TEMP grant must fail even when data hashes match。Docker context is explicitly configurable or safely discovered，rejects ambiguous/remote contexts，and never hard-codes a developer endpoint；required prerequisites fail rather than skip，using only sanitized fixtures.
...
Expected: the exact two persistent matrices plus all migration/recovery assertions PASS under PostgreSQL 18.4 TLS with zero required-test skips，all failed transactions roll back and all test-only hooks are restored before cleanup。The historical accepted exact-36 Task 1 used the then-current control/migration/application three-DSN wrapper only；that is not Source Gate successor evidence。After capability-identity harness merge，A2a and every successor migration/recovery G2 require all five control/migration/application/sealer/admitter DSNs：exact-36 must prove both capability application connections rejected，exact-38 must pass both full capability probes，and a missing/unreachable wrapper root or any DSN is an unmet prerequisite；a diagnostic Skip is never completion evidence。
...
After A2a the corrected migration will own the previous unordered 35 routine identities plus exact non-overloaded `public.asset_catalog_lock_exact_service_binding(uuid,uuid,uuid,uuid)`、`public.asset_catalog_seal_qualification_receipt(uuid,uuid,uuid,uuid,bigint,bigint,text,timestamp with time zone,timestamp with time zone,text)` and `public.asset_catalog_admit_source_gate(uuid,uuid,uuid,uuid,bigint,bigint)`；there will be no overload or 39th routine。All three are `RETURNS boolean LANGUAGE plpgsql VOLATILE STRICT PARALLEL UNSAFE SECURITY DEFINER`，owned by `aiops_schema_owner`，fixed `search_path=pg_catalog, public, pg_temp` and require `transaction_isolation='serializable'`/`transaction_read_only='off'`。The parent lock retains exact Service→binding semantics；the two Source Gate primitives both lock target Run→receipt/UUID-ordered prior Runs→Source→published Revision and append Audit/Outbox last。Seal compares Run version/fence；on first write it requires issued逐值等于同一 transaction的 `pg_catalog.transaction_timestamp()`、不早于 durable cleanup receipt，enforces `expiry <= issued + 24 hours` and canary expiry不晚于 prior HA expiry，then after durable `REVOKED` cleanup derives receipt/HA、fixed terminal/capacity/fence closure and `TERMINAL_COMMITTED` Audit without Source/Outbox mutation。已封存 exact tuple可在后续 transaction零写 replay，changed replay拒绝；admit compares Source version/gate revision and derives pointer/status/epoch/open Audit/Outbox，never caller decision or payload。Runtime/workload cannot execute either；exact session guards and ACL permit only sealer→seal and admitter→admit。The reviewed trigger manifest remains existing 39 plus bucket mutation/delete/truncate、permit immutable/delete/truncate and the one named Source Gate deferred trigger，exactly 46 when the 3+23 successor is present。Down takes the exact 20-relation set（six prerequisites + Audit + Outbox + twelve owned）in one `ACCESS EXCLUSIVE ... NOWAIT` statement，drops the existing cycle/Gate FKs and all successor triggers/functions before their dependencies，then child-first `service_asset_bindings→asset_relationships→asset_conflicts→asset_type_details→assets→asset_observations→asset_source_limit_permits→asset_source_limit_buckets→asset_source_runs→asset_source_revision_authorities→asset_source_revisions→asset_sources` before dependent typed helpers；no `CASCADE`，and any member conflict is `55P03` before change。
