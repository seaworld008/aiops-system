...
`M1A-asset-governance-repository` completed Task 3 in PR #38. The old Task 4 `Store.ReconcileBatch`/caller-supplied `CursorAfterHash`/caller-supplied `PageDigest` transaction sketch below remains **not executable**: it lacks checkpoint ciphertext、opaque key ID and typed AAD while requiring those facts to commit atomically.

Task 4 is now split by dependency and file ownership. `M1B-asset-source-read-projection` owns only `internal/assetcatalog/postgres/source_reads.go`、`source_reads_test.go` and `source_reads_integration_test.go` and implements the already frozen `assetcatalog.SourceReadRepository`; it owns no page mutation or public contract change. In parallel, `M1C-discovery-data-plane-contract` owns `internal/assetdiscovery/contracts.go`、`contracts_test.go`、`internal/discoverysource/contracts.go` and `contracts_test.go`：the first package owns pure normalized item/relation/freshness/provenance types formerly embedded in this Task 4 sketch, while the second owns Task 13's closed Provider outcome/checkpoint/request contract. This four-file contract foundation has no Reconciler、SQL、Queue、Worker、Credential Runtime or Provider network implementation, and Task 4 mutation later consumes rather than redeclares it.

M1C 与 M1D checkpoint foundation 已合并。mutation portion 先由 [M1E0 corrective](./12-m1e0-relation-page-corrective.md)关闭连续 canonical empty relation pages 的 `000015` trigger 冲突，再由快速计划 §12 指向的 [M1E 原子页提交任务包](./13-m1e-page-commit-transaction.md)接管：恰好新增 `discoverysource/page_commit.go`、对应 contract test，以及 PostgreSQL `page_committer.go`、包内 `page_projection.go`、真实 integration test 五个文件。M1E 同一 owner 同时拥有 Task 4 projection helper 与 Pack 09 `PageCommitter`，不组合 queue lifecycle/cleanup/limiter，也不修改 migration、现有 Repository 或 M1C/M1D 文件。

In M1E, `PageCommitter` is the sole serializable transaction owner. The Worker keeps the concrete Provider checkpoint only in the existing process-local `discoverysource.Page`；`PageCommitCoordinates` carries only safe Scope/Source/Run/page identity，sealed fence remains a separate argument。`CheckpointCodec.Seal` produces ciphertext/key ID/hash with exact typed AAD outside SQL；the transaction then locks and revalidates Run/Source/revision/fence/checkpoint-before, invokes a package-private projection helper, persists projection、sealed checkpoint、server-computed page/relation digests、safe receipts and checkpoint CAS, revalidates the fence, and commits once. No public Batch carries raw fence、checkpoint plaintext、caller-owned cursor-after hash or caller-owned page digest；no exported API exposes `pgx.Tx`，and no Repository transaction may nest inside `PageCommitter`。The exact ABI、five-file boundary、canonical ordering、empty relation page、error mapping and G2 evidence are owned only by the M1E task pack and must not be redefined here.
...
The three M1B Source-read files and the two pure `internal/assetdiscovery/contracts*` M1C files remain accepted history. M1E admits only its exact five new files；no separate exported Reconciler、`Store.ReconcileBatch` or MANUAL-only path may be used to claim discovery reconciliation complete。Queue/Worker/Provider and production availability remain `NOT_STARTED/UNAVAILABLE/CLOSED`。
