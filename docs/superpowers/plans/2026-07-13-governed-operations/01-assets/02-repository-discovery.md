...
The three M1B Source-read files and the two pure `internal/assetdiscovery/contracts*` M1C files are the only Task 4-owned production/test files currently admitted. Until the later mutation/PageCommitter file/interface manifest is merged, Reconciler and PostgreSQL discovery mutation files remain `NOT_STARTED`; no MANUAL-only path may be used to claim discovery reconciliation complete.
