...
6. [06-source-external-cmdb.md](./06-source-external-cmdb.md) — 实现固定 External CMDB Catalog v1 协议；Task 18A 只拥有 Provider paging/checkpoint，Task 18B 在 Pack 09 provider-neutral Worker core 合并后只拥有 CMDB durable reconciliation/lifecycle integration 和唯一 neutral metadata descriptor，Task 19A 单独装配 Control Plane profile/validation admission并把 non-MANUAL publication 保持关闭；reachability docs、第一次 manager contract sync、source-gate capability-identity harness、pre-A2a exact-2 test boundary 与第二次 manager evidence sync 合并后，fresh Task 19A2a 才冻结 exact 3+23/38-routine ABI，post-A2a exact-2 validation corrective 合并后 Task 19A2b/19A2c 再依次实现 persistence/current trust rechecks 与隔离 sealer/admitter connector、复用 Task 28A Worker seam 的私有 API/production，Task 19B 才拥有 CMDB canary/evaluator/AdmitGate operating proof。
...
Task 18B 与 Task 27/28 的权威顺序现固定为：已合并 PageCommitter/Queue/process-local CleanupBroker/Limiter → [Task 28A provider-neutral Worker core](./09-discovery-worker-ha-e2e.md#task-28a-provider-neutral-worker-core-and-claim-runtime-seam) → [External CMDB Task 18B](./06-source-external-cmdb.md#task-18b-external-cmdb-durable-reconciliation-and-lifecycle-integration) provider-specific 真库集成 + neutral descriptor → [Task 28B recoverable session transport](./09-discovery-worker-ha-e2e.md#task-28b-recoverable-cleanup-session-transport-and-attempt-authority) → [Task 28C production](./09-discovery-worker-ha-e2e.md#task-28c-production-constructor-and-provider-registry)。已合并 Task 28C/19A 后，Source Gate successor contract 严格串行为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → fresh Task 19A2a → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；具体入口见 [pre-A2a/Task 19A2a/19A2b/19A2c/19B](./06-source-external-cmdb.md) 与 [Task 29A/29B](./09-discovery-worker-ha-e2e.md)。两次 exact-3 manager-only PR 都只修改 `docs/status/current.md`、`docs/superpowers/plans/2026-07-13-governed-operations/coverage-matrix.md` 与 `docs/superpowers/plans/2026-07-13-governed-operations/01-assets/09-discovery-worker-ha-e2e.md`：第一次先修正唯一当前入口，第二次记录 capability/pre-A2a merged evidence并发布 formal A2a；均不改 ABI、业务完成度或 checkbox。capability-identity harness 只修改 CI/env/local/recovery 的 exact 8 files并预置隔离身份/future-gated ACL helper，不授予函数；pre-A2a exact-2 只修改两个已合并 fixture/test-boundary 文件。三批 A2 files 分别 12/11/23 files，由 fresh window 独立真实 G2/PR/merge且两两零重叠；fixture 或旧 dirty worktree不能反向成为 ABI 事实源。19A2c 必须复用唯一 Task 28A loop，seam 不足先做四文件 sequential C0 corrective；Task 29A 只消费 19A2b durable facts、19A2c sole signer 与 Task 28C observer seam，seam 不足先做冻结六文件 corrective。任何后继只读最新 `main` 已合并接口。
...
- 当前唯一顺序为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → fresh Task 19A2a → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。两次 manager exact-3 分别先修正唯一当前入口、再记录两个前置 Batch 的 merged evidence并发布 formal A2a；capability harness只预置身份/DSN/future-gated ACL helper，pre-A2a exact-2只冻结 exact-38 routine/ACL。formal A2a必须从第二次 sync 后最新`main` fresh重启，不读取本次未提交尝试。A2a合并后 post-A2a exact-2仍只拥有`internal/assetcatalog/validation.go`与新建`internal/assetcatalog/validation_test.go`；A2b实现两条 primitive 的 Repository consumer、narrow executor与 current rechecks，A2c再装配 distinct fixed-role connectors、fixed mTLS API、immutable verifier registry/sole sink+signer。文件所有权零重叠且只有 A2c 可复用 Task 28A 唯一 loop，不得创建独立 qualification runner。Qualification不得进入 browser `effective_actions`、普通`RequestSync`、PageCommitter或 Source checkpoint/success projection。Task29A `ha.go`只验证 A2b Repository-loaded facts并经 A2c signer封存；Task19B才产生 External CMDB canary/evaluator；Task29B只读聚合 receipts。
