...
Task 18B 与 Task 27/28 的权威顺序现固定为：已合并 PageCommitter/Queue/process-local CleanupBroker/Limiter → [Task 28A provider-neutral Worker core](./09-discovery-worker-ha-e2e.md#task-28a-provider-neutral-worker-core-and-claim-runtime-seam) → [External CMDB Task 18B](./06-source-external-cmdb.md#task-18b-external-cmdb-durable-reconciliation-and-lifecycle-integration) provider-specific 真库集成 + neutral descriptor → [Task 28B recoverable session transport](./09-discovery-worker-ha-e2e.md#task-28b-recoverable-cleanup-session-transport-and-attempt-authority) → [Task 28C production](./09-discovery-worker-ha-e2e.md#task-28c-production-constructor-and-provider-registry)。已合并 Task 28C/19A 后，Source Gate successor contract 严格串行为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；具体入口见 [pre-A2a/Task 19A2a/19A2b/19A2c/19B](./06-source-external-cmdb.md) 与 [Task 29A/29B](./09-discovery-worker-ha-e2e.md)。global routine ACL corrective把原exact8与status/coverage/Pack09组成同一exact11原子提交，进入`main`时合同与入口同时生效；本exact11不实现A2a/G2、不改ABI、业务完成度或checkbox。上一fresh A2a因predecessor72默认PUBLIC让两个capability identity产生合计144条额外effective edge而`STOPPED/NOT PASS`，其未提交worktree不得续用或复制。predecessor 基线 direct grant 为0而normalized PUBLIC exact72；fresh A2a up必须消费Pack06唯一exact72 manifest与production digest，新增内容寻址、owner-grantor/non-grantable的runtime→predecessor exact72 direct manifest，down先精确撤销再恢复PUBLIC72。pre-A2a exact-2 global helper保持安全真值；A2a exact-12 integration test负责 pre-up72、up110 direct/effective与edge digest、unexpected111、wrong predecessor ACL、down72 restored、up/down/up、五身份、application identity关键DML/trigger/helper路径与required双实例dump/restore。三批 A2 files 分别12/11/23 files，由fresh window独立真实G2/PR/merge且两两零重叠；exact11合并后的唯一入口是从届时最新`origin/main`新建fresh A2a，fixture或旧dirty worktree不能反向成为ABI事实源。19A2c必须复用唯一Task28A loop，Task29A seam不足时先做冻结corrective。任何后继只读最新`main`已合并接口，所有Source/Provider/Worker保持`NOT_STARTED/UNAVAILABLE/CLOSED`。
...
- 当前唯一顺序为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`。global routine ACL exact11原子同步原exact8与status/coverage/Pack09；其提交进入`main`后唯一入口是从最新`origin/main`新建fresh A2a。上一A2a因PUBLIC72产生144条额外capability edge而`STOPPED/NOT PASS`，旧dirty/WIP不得读取、续用或复制；本corrective不实现A2a/G2、不改完成度或能力。A2a合并后 post-A2a exact-2仍只拥有`internal/assetcatalog/validation.go`与新建`internal/assetcatalog/validation_test.go`；A2b/A2c文件零重叠且只有A2c可复用Task28A唯一loop。Qualification不得进入browser`effective_actions`、普通`RequestSync`、PageCommitter或Source checkpoint/success projection；所有Source/Provider/Worker保持`NOT_STARTED/UNAVAILABLE/CLOSED`。
