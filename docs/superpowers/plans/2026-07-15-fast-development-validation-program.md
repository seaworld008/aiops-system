...
当前 Batch 固定为 `M0-asset-domain-contract`：

1. 保留 Task 2 已写的有价值 RED，但先移除 MANUAL checkpoint、初始 `GateRevision=0`、CANCELLED closure、OwnerGroup/Fence 等已确认的测试假约束；
2. 定向修复迁移只接受 `SINGLE_ENVIRONMENT / EXPLICIT_ITEM_ENVIRONMENT`，明确拒绝旧 `MULTI_ENVIRONMENT`；
3. 同一 Batch 完成 Task 2 的 Auth、领域模型、validation、lifecycle、repository interface 和 lease/fence 最小正确实现；
4. 运行受影响 Go 包、BindingDigest PostgreSQL parity、迁移定向 up/admission、G1 和 G2；
5. 不运行与该变更无影响关系的双实例 dump/restore、全仓 race、全 Provider E2E 或多轮独立 test-only review；
6. 合并后状态只记为 `BUILT_CLOSED`，资产运行能力继续 `UNAVAILABLE`。
