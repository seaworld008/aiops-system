...
当前从最新 `origin/main` 并行创建两个 fresh 窗口：

- [Pack 03 Tasks 5–6](2026-07-13-governed-operations/01-assets/03-mapping-auth-api.md) 聚合为 `M1F-mapping-management-boundary`，只实现 Mapping PostgreSQL Repository、资产权限和 Management 接口，不修改 migration/OpenAPI/Web/Queue/CleanupBroker。
- `Limiter-persistence-contract-corrective` 先修复 [Pack 09 Task 27](2026-07-13-governed-operations/01-assets/09-discovery-worker-ha-e2e.md) 的 C0 前置缺口：现有 `000015` 无法精确表达 Source/Workspace/Provider 三层独立 bucket、active permit 与幂等 Release。该轨道必须先完成权威契约和独立复核，再做 `000015` PostgreSQL RED/GREEN；不得直接恢复原三个 Limiter 文件实现。

Task 28 Discovery Worker、Provider 和生产装配必须等待 Limiter stable `Produces` 以及各自前置任务；所有后继关闭态 Batch 最多记为 `BUILT_CLOSED`，资产运行能力继续 `UNAVAILABLE`。
