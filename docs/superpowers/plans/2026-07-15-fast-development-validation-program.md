...
- Source Gate 轨道 successor contract 严格串行 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；PR、fixture 或 dirty worktree 不能反向成为契约源。global routine ACL exact11 已由 PR #152 合并到 `main@d8d632f453929fbe056dc499a6d64144d83011bf`。PR #152 后的 fresh A2a 确认 pre-A2a fixture 无条件合成 exact3/trigger/drop，与 formal `000015` 必须内建的同一对象冲突；该 partial A2a 与更早尝试均为 `STOPPED/NOT PASS`，未提交 worktree/WIP/snapshot 永不作为输入。pre-A2a exact-2 global helper不得修改或收窄；当前唯一入口是下述 exact9，合并后才可从最新`origin/main`新建fresh A2a exact-12。A2a integration test仍必须覆盖pre-up72、up110 direct/effective与runtime exact72 edge digest、unexpected111、wrong predecessor ACL、down72 restored、up/down/up、五身份、application identity关键DML/trigger/helper路径与required双实例dump/restore；post-A2a exact-2仍只拥有`internal/assetcatalog/validation.go`与新建`internal/assetcatalog/validation_test.go`。
- pre-A2a formal-fixture compatibility exact9 的 Phase A 唯一拥有八份权威文档并先独立复核，Phase B 才唯一修改 test-only `internal/assetcatalog/postgres/migration_corrective_test.go`。Phase B 必须先用已-formal up/down fixture 状态产生真实 RED，再使 construction state-aware/idempotent：baseline 三对象全 absent 时合成，formal 三对象全 exact present 时验证后复用；partial、duplicate、renamed、wrong type/trigger/relation/lifecycle、dynamic DDL或up/down不一致全部fail closed。不得删除或弱化既有 adversarial matrix、global exact72/110、owned exact38、ACL/owner/grantor/grantability/C-order/down manifest。Task19A2a exact12禁止修改该helper；exact9只运行定向normal/race、受影响完整runnable unit/static suite与fresh G1，不以PostgreSQL G2冒充A2a，也不改变completion、checkbox、Provider/Worker/Capability或G2/G4状态。
...
当前 Source Gate 前置 reachability、capability-identity harness、pre-A2a exact-2 与其 manager evidence sync 已按序合并；global helper已证明安全目标是全部 application-schema public routines，而不是仅`000015` owned routines。静态复算固定 `000001..000014` reviewed predecessor为exact72，A2a owned manifest为exact38，因此global closure为exact110。global routine ACL exact11 已由 PR #152 合并为 `main@d8d632f453929fbe056dc499a6d64144d83011bf`；其后 fresh A2a 暴露 formal migration 与 test fixture 重复 exact3/trigger/drop 的新 blocker并保持`STOPPED/NOT PASS`。当前唯一立即执行 Batch 是 pre-A2a formal-fixture compatibility exact9 corrective：先 exact8 docs 独立复核，再 exact1 test-only RED→GREEN/G1/独立复核/merge；在它合并前不得启动 fresh Task19A2a exact12，不得续用或复制任何旧未提交实现、WIP或snapshot。所有后继最多`BUILT_CLOSED`，真实资格前所有Source/Provider/Worker继续`NOT_STARTED/UNAVAILABLE/CLOSED`。
