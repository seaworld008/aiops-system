...
- Source Gate 轨道 successor contract 严格串行 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；PR、fixture 或 dirty worktree 不能反向成为契约源。global routine ACL corrective把原exact8与`docs/status/current.md`、`coverage-matrix.md`、Pack09组成同一exact11原子提交，进入`main`时合同与当前入口同时生效，不存在后继manager sync窗口。上一fresh A2a因predecessor72默认PUBLIC `EXECUTE`为两个capability identity产生合计144条额外effective edge而`STOPPED/NOT PASS`，其未提交worktree不得续用或复制；本exact11不实现A2a/G2、不改业务完成度、checkbox或能力。pre-A2a exact-2 global helper不得修改或收窄；exact11合并后的唯一下一入口是从届时最新`origin/main`新建fresh A2a exact-12，消费Pack06唯一exact72 manifest与production digest，其integration test必须覆盖pre-up72、up110 direct/effective与runtime exact72 edge digest、unexpected111、wrong predecessor ACL、down72 restored、up/down/up、五身份、application identity关键DML/trigger/helper路径与required双实例dump/restore。post-A2a exact-2仍只拥有`internal/assetcatalog/validation.go`与新建`internal/assetcatalog/validation_test.go`。
...
当前 Source Gate 前置 reachability、capability-identity harness、pre-A2a exact-2 与其 manager evidence sync 已按序合并；global helper已证明安全目标是全部 application-schema public routines，而不是仅`000015` owned routines。静态复算固定 `000001..000014` reviewed predecessor为exact72，A2a owned manifest为exact38，因此global closure为exact110。上一fresh A2a因默认PUBLIC形成144条额外capability edge而停止；本global routine ACL exact11 corrective只修权威合同并原子同步current/coverage/Pack09入口，不执行或验收A2a/G2。该exact11提交出现在`main`后，唯一下一入口才是从届时最新`origin/main`创建fresh Task19A2a exact12；不得续用或复制旧未提交实现、WIP或snapshot。所有后继最多`BUILT_CLOSED`，真实资格前所有Source/Provider/Worker继续`NOT_STARTED/UNAVAILABLE/CLOSED`。
