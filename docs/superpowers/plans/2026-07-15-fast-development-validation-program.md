...
- Source Gate 的 exact 3+23 nullable-column ABI、four-column identity FK、deferred closure、15-frame signed receipt，以及 terminal seal 与 gate-open 的双 serializable transaction；
...
- Source Gate 轨道 successor contract 严格串行 `docs-contract corrective → 修正/重基 PR #134 → merge PR #134 → fresh Task 19A2a → exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；PR、fixture 或 dirty worktree 不能反向成为契约源。exact-2 是只拥有 `internal/assetcatalog/validation.go` 与新建 `validation_test.go` 的 sequential C0 domain-validation corrective，不是 migration/persistence/Worker Batch。本 corrective 合并后由主管理窗口另建 exact-3 manager-only PR，仅修改 `docs/status/current.md`、governed-operations `coverage-matrix.md`、Phase 1 `09-discovery-worker-ha-e2e.md` 来同步同一顺序/`Consumes`，不改 ABI、完成度或 checkbox；exact-3 合并前 admission 停在 docs corrective，PR #134 与 A2a 均不得启动。
...
当前 Source Gate 的下一入口是独立五文档 C0 corrective。它先冻结 `asset_sources` exactly 3 nullable pointer columns、`asset_source_runs` exactly 13 qualification + 10 HA nullable columns、four-column deferred identity FK、named deferred closure、exact 15-frame receipt 和 `A2a terminal seal → separate A2b AdmitGate` 生命周期；`source.gate_revision=qualification_run.gate_revision+1` 只约束首次开门 commit，后续同 binding rollover 保留 current pointer，并以无间隙 terminal receipt chain 解释更高 epoch，`SUSPENDED`/drift/expiry 则原子清空。随后才按上节唯一串行顺序推进。migration-owner 只可在 disposable `_test.go` database 中构造 structurally-valid closure/recovery fixture，且 synthetic pointer + `AVAILABLE` 必须同一最终 serializable transaction；application/workload direct write 必须 `42501`，fixture 不计 A2a/A2b/G2/G4 或 availability 证据。所有后继最多为 `BUILT_CLOSED`，真实资格前资产运行能力继续 `UNAVAILABLE/CLOSED`。
