...
`M0-asset-domain-contract` 已通过 PR #34 合并并记为 `BUILT_CLOSED / UNAVAILABLE`。当前 Batch 固定为 `M1A-asset-governance-repository`：

1. 只执行 Phase 1 Pack 02 Task 3，完成复合 Scope PostgreSQL Repository、MANUAL 原子治理写、receipt-first 幂等 replay、安全读模型以及 Audit/Outbox/Observation/Type Detail 同事务闭包；
2. 保留 Scope、SERIALIZABLE、fence、idempotency、governance 不被覆盖等 C0/C1 关键行为 RED，删除脆弱的纯 SQL 文本镜像门；
3. G2 运行受影响 Go 包、并发敏感路径定向 race、真实 PostgreSQL 事务/回滚/幂等测试、G1 和一次最终 P0/P1 复核；
4. 不修改 migration、OpenAPI/生成类型、Web、Provider、Source mutation 或 status，不运行无影响关系的全仓 race、双实例恢复、全 Provider E2E；
5. Pack 02 Task 4 因 checkpoint/事务所有权 P0 从 M1A 拆出，保持 `NOT_STARTED`，与 Pack 09 Task 27 合并为后继 `M1B-discovery-page-commit`；
6. M1B 的 `PageCommitter` 是唯一 serializable transaction owner：它消费 Worker 内存中的具体 next checkpoint，通过 typed `CheckpointAAD` 在 SQL 外 seal，进入事务后锁定并复验 Source/Run/fence/checkpoint-before，在同一事务调用私有 projection、持久化 ciphertext/key ID/hash、page receipt 和 checkpoint CAS；receipt-first replay 还必须用域分离 keyed checkpoint identity 区分相同事实下不同的 `NextCheckpoint`，不能把随机 ciphertext hash 当唯一 replay key。旧 Task 4 中由 caller 提交 `CursorAfterHash/PageDigest`、Repository 单独提交 projection 或嵌套事务的解释全部作废；
7. M1A/M1B 合并状态最多记为 `BUILT_CLOSED`，资产运行能力继续 `UNAVAILABLE`。
