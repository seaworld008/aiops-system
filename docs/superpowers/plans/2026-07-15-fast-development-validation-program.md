...
- Source Gate 的 exact 3+23 nullable-column ABI、four-column identity FK、deferred structural closure、15-frame signed receipt、first-write same-transaction DB issued time与 cleanup ordering、closed evidence-kind maximum TTL（当前两 kind 均 24 hours）、terminal seal 与 gate-open 的双 serializable transaction，以及仅两条 fixed `SECURITY DEFINER` primitive和相互隔离的 sealer/admitter DB identities；
- Source Gate routine ACL 的双层内容寻址合同：`000015` owned exact38与application schema global exact110（predecessor72+Asset38）。唯一C-order predecessor identities与production digest见 [Pack06 canonical predecessor exact72 runtime EXECUTE manifest](./2026-07-13-governed-operations/01-assets/06-source-external-cmdb.md#canonical-predecessor-exact72-runtime-execute-manifest)；本快速计划不复制列表。基线direct grant0、normalized PUBLIC72；migration/admission/test必须逐项匹配该固定列表并使用该production常量，不得运行时生成后接受。未知73rd、缺失、overload或ACL/owner/grantor/grantability漂移整笔回滚。up/down只可显式枚举，不得使用`ON ALL FUNCTIONS IN SCHEMA`或schema-wide grant/revoke；up新增runtime→predecessor exact72 owner-grantor/non-grantable edges，down先撤销它们再恢复PUBLIC72，最终catalog/ACL等于pre-up；
...
一个局部纠错只有在代码地图/影响分析证明涉及恢复格式、归档 owner/ACL、持久数据解释或跨版本兼容时，才升级到恢复/双实例门。Source Gate global routine ACL corrective 已确认同时改变归档 ACL 与 down 恢复语义，因此 fresh A2a 的双实例 dump/restore recovery 是 required G2，不得继续 deferred。
...
- Source Gate 轨道 successor contract 严格串行 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → pre-A2a identity-FK fixture compatibility exact-9B docs-only contract checkpoint → [DEVELOPMENT_PAUSED] → resumed fresh test-only identity-FK fixture corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；PR、fixture 或 dirty worktree 不能反向成为契约源。global routine ACL exact11 已由 PR #152 合并到 `main@d8d632f453929fbe056dc499a6d64144d83011bf`，formal-fixture exact9 已由 PR #153 合并到 `main@c0b620e6fff9de0b746504f5fb7231fcb4a213c4`、tree `0af13374448f1386593291631e10a07add41440b`。PR #153 后的 fresh A2a 确认合法top-level identity FK被报`unreviewed gate-evidence ALTER`、合法inline named FK被报`unreviewed gate-evidence table element`；该partial A2a、停止的`cb00`与其他 dirty/stopped A2a worktree/WIP/snapshot均为`STOPPED/NOT PASS`且永不作为输入，任何未完成实现都不提交、不合并。pre-A2a exact-2 global helper不得修改或收窄；截至2026-07-23开发暂停，docs-only exact9B不解阻fresh A2a。A2a integration test仍必须覆盖pre-up72、up110 direct/effective与runtime exact72 edge digest、unexpected111、wrong predecessor ACL、down72 restored、up/down/up、五身份、application identity关键DML/trigger/helper路径与required双实例dump/restore；post-A2a exact-2仍只拥有`internal/assetcatalog/validation.go`与新建`internal/assetcatalog/validation_test.go`。
- pre-A2a identity-FK fixture compatibility exact9B 当前只把八份权威文档冻结为 docs-only 暂停检查点；原 Phase B test-only实现永久停止于本轮并保持`NOT_STARTED`，不是本检查点或其后文档PR的交付。冻结的未来合同要求从届时最新`origin/main`创建fresh、独立test-only corrective，先分别用inline named constraint与唯一精确top-level`ALTER TABLE public.asset_sources ADD CONSTRAINT asset_sources_gate_evidence_run_fk`的已-formal up/down状态产生真实RED，再实现严格FK parser/state-aware可逆baseline extraction。最终formal必须有且仅有named `asset_sources_gate_evidence_run_fk` exact four-column `DEFERRABLE INITIALLY DEFERRED` FK，source/reference column order、table/schema和mapping逐字精确；baseline无该FK，formal→baseline→formal重建后重验。missing/partial/duplicate、wrong/quoted alias name、wrong columns/order/table/schema、6-column、digest/expiry identity、`NOT DEFERRABLE`/`INITIALLY IMMEDIATE`、dynamic DDL、除该唯一精确top-level创建语句外的任何ALTER、后续任何DROP/VALIDATE/rename/disable lifecycle、额外gate constraint或up/down mismatch全部fail closed。不得使用宽松contains、只删first match、隐藏标识符或跳过/弱化PR #153 exact3/trigger/drop auxiliary matrix、既有adversarial matrix、global exact72/110、owned exact38、ACL/owner/grantor/grantability/C-order/down manifest。恢复后的corrective须完成定向normal/race、受影响完整runnable unit/static suite、fresh G1、独立复核、PR与合并，之后才允许fresh Task19A2a exact12；A2a禁止修改该helper。docs-only exact9B不以PostgreSQL G2冒充A2a，也不改变completion、checkbox、Provider/Worker/Capability或G2/G4状态。
...
### 9.1 Codex Review 非阻塞闭环

- 每个 PR 都检查 Codex Review 和 `reviewThreads`，记录评论到达时间，并区分 `unresolved`、`outdated`；只处理经复现仍有效的问题。
- 合并前确认的 P0/P1 阻断当前 PR；有效 P2 若在当前文件所有权内且可由小改动和定向测试关闭，则在当前 PR 修复，否则登记 owner/next Batch，且不阻断无文件重叠、无依赖关系的其他 Batch；P3 记录 backlog。
- 合并后才到达的反馈不追溯宣告原合并失败：P0/P1 立即建立 corrective 并阻断依赖面，P2 进入下一个非阻塞维护/纠偏 Batch，P3 记录 backlog。
- 每条反馈记录 PR/thread URL、分级、复现结论、owner、RED/GREEN 和合并修复证据；修复 PR 合并前不得把对应 thread 虚假标记为 resolved。
- 等待 Codex Review 或处理低级别修复不得让独立实现轨空转，但任何真实 P0/P1 都必须进入上述阻断闭环。
...
M1E、Queue lifecycle 与 CleanupBroker 已分别通过 PR #53、#55、#57 合并；它们的公开 ABI、原子页提交、Queue 状态机和进程内 cleanup boundary 都成为 `BUILT_CLOSED` 的稳定 `Produces`，运行能力仍为 `UNAVAILABLE/CLOSED`。

当前 Source Gate 前置 reachability、capability-identity harness、pre-A2a exact-2 与其 manager evidence sync 已按序合并；global helper已证明安全目标是全部 application-schema public routines，而不是仅`000015` owned routines。静态复算固定 `000001..000014` reviewed predecessor为exact72，A2a owned manifest为exact38，因此global closure为exact110。global routine ACL exact11 已由 PR #152 合并，formal-fixture exact9 已由 PR #153 合并为`main@c0b620e6fff9de0b746504f5fb7231fcb4a213c4`、tree`0af13374448f1386593291631e10a07add41440b`；其后fresh A2a暴露合法identity FK在fixture parser/extractor两条表达路径均被拒绝的新blocker并保持`STOPPED/NOT PASS`。截至2026-07-23，当前 Batch 只把pre-A2a identity-FK fixture compatibility exact9B的exact8文档合同冻结为docs-only暂停检查点并等待独立复核；开发已经暂停，原Phase B test-only实现`NOT_STARTED`且不是本交付，没有可提交或合并的实现。恢复后的唯一入口是从届时最新`origin/main`创建fresh、独立test-only identity-FK fixture corrective并完成RED→GREEN、完整验证、独立复核、PR/merge；其后才允许fresh Task19A2a exact12。不得续用或复制停止的`cb00`及任何旧dirty/stopped A2a实现、WIP或snapshot。所有后继最多`BUILT_CLOSED`，真实资格前所有Source/Provider/Worker继续`NOT_STARTED/UNAVAILABLE/CLOSED`。
