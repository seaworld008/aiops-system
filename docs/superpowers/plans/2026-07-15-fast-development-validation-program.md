...
- Source Gate 轨道 successor contract 严格串行 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → fresh Task 19A2a → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；PR、fixture 或 dirty worktree 不能反向成为契约源。两次 manager exact-3 都只拥有 `docs/status/current.md`、`docs/superpowers/plans/2026-07-13-governed-operations/coverage-matrix.md`、`docs/superpowers/plans/2026-07-13-governed-operations/01-assets/09-discovery-worker-ha-e2e.md`：第一次先修正唯一当前入口，第二次记录前置 merged evidence并发布 formal A2a。capability harness 的 exact 8 files 逐字见 Pack 06；pre-A2a exact-2 只拥有 `internal/assetcatalog/postgres/migration_corrective_test.go` 与 `internal/assetcatalog/postgres/migration_closure_adversarial_integration_test.go`；post-A2a exact-2 只拥有 `internal/assetcatalog/validation.go` 与新建 `internal/assetcatalog/validation_test.go`；五项前置合并前 formal A2a 禁止重启。
...
当前 Source Gate 在 fresh formal A2a 的真实 Go/ACL/PostgreSQL RED 后发现三项 C0 reachability 缺口：protected columns 无 A2b 可达正向写 authority、`000015` 无 current signing-key/runtime 真值却被要求验签、qualification-required Profile 无冻结持久判别。该尝试保持未提交且不能成为后继输入。PR #143 已合并独立八文档 reachability corrective：保留 exact 3+23/four-column FK/15-frame receipt，只在既有 36 routines 上冻结两条窄 primitive，并将普通 workload 的 EXECUTE 关闭、分别交给无 membership/关系权限的 sealer/admitter identity。`RecordCleanup` 先持久 `REVOKED`；seal按 Run→prior Runs→Source→Revision锁序派生 receipt/HA/terminal/Audit，admit只在 A2b/A2c current trust复验后派生 pointer/open Audit/Outbox。A2a只证明 structural closure；exact published Profile三元判别留在 DB/A2b composite loader，`Source.Validate` 不伪造缺失字段。Source/Run protected DML按 exact边界保持`42501`，synthetic fixture/signature不计验签、A2a/A2b/G2/G4或 availability。PR #144 已合并第一次 manager exact-3 contract sync；当前唯一入口是从最新 `origin/main` 的 fresh clean worktree 启动 capability-identity harness C0，严格只拥有 Pack 06 冻结的 exact 8 files，不修改 migration/OpenAPI/status、不授予函数、不打开 pool 或能力。随后按上节串行推进；所有后继最多`BUILT_CLOSED`，真实资格前能力继续`UNAVAILABLE/CLOSED`。
