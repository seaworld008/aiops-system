...
`M0-asset-domain-contract`（PR #34）和 `M1A-asset-governance-repository`（PR #38）均已合并并保持 `BUILT_CLOSED / UNAVAILABLE`。当前从同一最新 `origin/main` 并行执行两个无文件交集的 Batch：

1. `M1B-asset-source-read-projection` 只实现 Pack 02 的独立只读子切片，精确文件为 `internal/assetcatalog/postgres/source_reads.go`、`source_reads_test.go`、`source_reads_integration_test.go`；
2. M1B 直接实现 M0 已冻结的 `assetcatalog.SourceReadRepository`，不得修改 `internal/assetcatalog/types.go` 或 `repository.go`，不得创建 normalized fact 合同、Reconciler、PageCommitter、queue/checkpoint 或 Source mutation；
3. `M1C-discovery-data-plane-contract` 精确拥有 `internal/assetdiscovery/contracts.go`、`contracts_test.go`、`internal/discoverysource/contracts.go`、`contracts_test.go`；它把 normalized item/relation/freshness/provenance 与 closed Provider `Page|Delay`/checkpoint/request/limit/XOR 合同同批冻结，不创建 Reconciler、SQL、Queue、Worker、Credential Runtime 或 Provider 网络实现；
4. M1B 保留复合 Source Scope、完整 authority-set 子集授权、restricted-empty、MANUAL 单 Environment usage、稳定 keyset/QueryDigest、safe columns 与 Secret/checkpoint/fence 排除；M1C 保留 closed sum type、XOR、typed-nil/pointer/新定义 wrapper 拒绝、checkpoint/事实不可序列化泄漏与输入上限/secret-shaped negative；Go 真 alias 与原 concrete type 身份相同，不能也无需伪造可区分门；
5. 两批各自只运行受影响测试、G1/G2 和一次最终 P0/P1 复核，独立 PR；任一先合并均可，另一批合并前必须从新 `origin/main` 复验 base/接口无漂移，不运行无关 race、恢复、Provider、HA、浏览器或发布资格；
6. 原计划把 Task 4 与 Task 27 的 21+ 文件放进一个 `M1B-discovery-page-commit`，已按 L/XL 拆分规则作废。M1B/M1C 合并后，后继顺序固定为 queue/sealed-fence/checkpoint 基础 → 单一 owner 的 Task 4 mutation + `PageCommitter` 原子页提交 → queue lifecycle/cleanup/limiter；
7. PageCommitter 写 Batch 的精确文件/接口 manifest 必须在该 Batch 开始前合并。Receipt-first lookup 必须发生在任何 seal/reseal 之前；只有确定为新页才 seal 一次并跨 bounded SQL retry 复用。它仍是唯一 serializable page transaction owner；旧 caller-owned `CursorAfterHash/PageDigest`、Repository 独立提交 projection、MANUAL-only 假绿或任何嵌套事务解释全部作废；
8. M1B/M1C 及上述后继关闭态 Batch 最多记为 `BUILT_CLOSED`，资产运行能力继续 `UNAVAILABLE`。
