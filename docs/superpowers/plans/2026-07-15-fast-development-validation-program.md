...
`M0-asset-domain-contract`（PR #34）、`M1A-asset-governance-repository`（PR #38）、`M1B-asset-source-read-projection`（PR #41）和 `M1C-discovery-data-plane-contract`（PR #42）均已合并并保持 `BUILT_CLOSED / UNAVAILABLE`。当前执行一个 Small/Medium C0 Batch：

1. `M1D-checkpoint-codec-foundation` 精确拥有 `internal/discoverycheckpoint/codec.go`、`codec_test.go`，不得修改其他文件；
2. M1D 只消费 M1C `discoverysource.Checkpoint` 与已冻结 Source/Revision 安全元数据，不创建 `internal/discoveryqueue`，也不复制现有 `assetcatalog.LeaseFence`、raw token 或 checkpoint 容器；
3. `discoverycheckpoint/codec.go` 唯一拥有 Pack 09 已定值的 typed `CheckpointAAD`、DB-only `SealedCheckpoint`、safe `ReplayIdentity`、in-memory keyring 和 `CheckpointCodec.Seal/Open/ReplayIdentity` 完整 ABI；
4. AES-256-GCM additional data 保持 Pack 01 九字段 exact encoding；profile code 通过 profile-bound HKDF AEAD 子密钥认证，raw checkpoint 直接作为 plaintext，使 `65,507 + 29 = 65,536` 的数据库 envelope 上限仍可实现。Replay 使用另一固定 HKDF info 派生 HMAC 子密钥，master/AEAD/replay key 不复用；
5. M1D C0 测试必须真实证明九字段逐项 tamper、NULL/empty、wrong profile/version/Scope/revision、missing retained key、key rotation、random nonce、65,507-byte 上限、raw key/checkpoint/AAD 的 JSON/text/binary/log/format 排除和 AEAD/replay 子密钥分离；
6. M1D 只运行 `internal/discoverycheckpoint`、定向 race、G1/G2 和一次最终 P0/P1 复核；PostgreSQL、Queue、PageCommitter、Worker、Provider、HA、恢复、浏览器、安全与发布资格记为 deferred G3/G4；
7. M1D 合并后先合并 PageCommitter 写 Batch 的精确文件/接口 manifest，再实现唯一 transaction owner。Receipt-first lookup 必须发生在任何 seal/reseal 之前；只有确定为新页才 seal 一次并跨 bounded SQL retry 复用；
8. Queue 公共 ABI 与进程内 ClaimResult 的 sealed `LeaseFence` 只在真实 PostgreSQL lifecycle 消费点一起定值；持久化/可序列化 Queue payload 永不携带 fence，而 ClaimResult 必须保持进程内、不可序列化、不可日志化。Queue lifecycle/cleanup/limiter 再按文件所有权分片收口；所有后继关闭态 Batch 最多记为 `BUILT_CLOSED`，资产运行能力继续 `UNAVAILABLE`。
