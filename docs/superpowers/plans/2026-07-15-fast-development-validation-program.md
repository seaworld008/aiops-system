...
M1E contract RED 发现已合并 M1C normalized-fact ABI 与既有 Asset/SQL 契约不闭合：DisplayName 的 512 上限高于持久层 256，跨 Environment `ObservedRelation` 缺少规范和数据库要求的显式 Policy Reference。当前先执行 [M1C1 — Normalized Fact Contract Corrective](2026-07-13-governed-operations/01-assets/14-m1c1-normalized-fact-contract-corrective.md)，以原 M1C 四个 contract/test 文件的 C0 RED/GREEN 收敛公共输入并修正 page-byte accounting；M1E 暂停且不得猜测策略引用或把 opaque ID 语法合法当成策略许可。

M1C1 squash merge 后，从最新 `origin/main` 创建 fresh [M1E — Atomic Page Commit Transaction](2026-07-13-governed-operations/01-assets/13-m1e-page-commit-transaction.md) 窗口。M1E 精确拥有两个 `discoverysource/page_commit*` contract 文件与三个 PostgreSQL PageCommitter/projection/integration 文件；公开 ABI、receipt-first/single-Seal 顺序、canonical page/relation identity、FactPolicy resolver、one-transaction closure、错误 vocabulary 与 G2 证据只在该任务包定值，Pack 02/09 仅引用而不另造平行版本。
