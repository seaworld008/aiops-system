...
当前纠偏后的实现与合并顺序唯一为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；每步只消费最新 `main` 已合并契约，PR/fixture/旧 dirty worktree 或未合并实现不得反向成为事实源。global routine ACL corrective 原 exact 8 文档与 `docs/status/current.md`、`docs/superpowers/plans/2026-07-13-governed-operations/coverage-matrix.md`、`docs/superpowers/plans/2026-07-13-governed-operations/01-assets/09-discovery-worker-ha-e2e.md` 组成同一 exact 11 原子提交；该提交进入 `main` 时合同与当前入口同时生效，不存在后继 manager-only sync 窗口，也不改测试、migration、代码、OpenAPI、生成物、完成度、checkbox 或能力状态。capability-identity harness C0 只拥有 `.github/workflows/ci.yml`、`.env.example`、`scripts/with-local-postgres.sh`、`docs/operations/local-postgresql-development.md`、`internal/assetcatalog/postgres/migration_integration_test.go`、`internal/assetcatalog/postgres/recovery_container_test.go`、`internal/assetcatalog/postgres/migration_recovery_integration_test.go` 与 `internal/store/postgres/migrations_integration_test.go`，只预置隔离角色/DSN/证书、bidirectional future-gated application-DB ACL reconciliation和测试 identity，不授予函数或业务能力；只有 exact-38 owned manifest + exact-110 global ACL postflight进入 grant分支并随后通过 full admission，exact-36/down/unknown/partial都先 revoke并验 absent，unknown/partial再 fail closed。`.env.example` 同时预留空的 `AIOPS_SOURCE_GATE_ADMIT_DATABASE_URL` 与 `AIOPS_DISCOVERY_SOURCE_GATE_SEAL_DSN_FILE`，A2c 才装配。上一 fresh A2a 尝试因 predecessor exact 72 的默认 PUBLIC `EXECUTE` 让两个 capability identity各获得72条额外 effective edge、合计144条而停止；该未提交 worktree 不得续用或复制，本 exact11 只修文档合同与当前入口，不实现 A2a/G2。pre-A2a exact-2 的 global helper 继续作为不可收窄的安全真值；本 exact11 进入 `main` 后唯一下一入口是从届时最新 `origin/main` 新建 fresh Task 19A2a exact-12，消费 Pack 06 唯一 exact72 identity manifest及 production digest，并覆盖 global110/runtime72、up/down、五身份与 required 双实例 dump/restore。当前 A2a 保持 `STOPPED/NOT PASS`；所有 Source/Provider/Worker 保持 `NOT_STARTED/UNAVAILABLE/CLOSED`。
