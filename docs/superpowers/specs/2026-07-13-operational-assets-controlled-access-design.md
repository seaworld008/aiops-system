...
每份 qualification receipt 都是 terminal Run 上 append-only、content-addressed、签名且有时效的 safe tuple，复合绑定 Tenant/Workspace/Source、published revision/canonical binding、Provider/Profile descriptor、runtime manifest、opaque lab-binding digest、evidence kind、前序 receipt digests、结果与 expiry；receipt、API、Run、Audit、Outbox 和日志均不得包含 endpoint、Credential Reference 的值、credential、raw Provider payload/cursor、Header/Body 或错误正文。`TWO_WORKER_HA` 还必须由 Queue/run/Task28B receipts 生成并重载 distinct owner/takeover worker+process、exact fence takeover、restart、session recovery、cleanup 和 pre-terminal response-loss safe digests；caller、脚本、日志或 test fake 不能提交这些事实。唯一 immutable verifier registry/outcome sink reloads durable facts，HA verifier只能返回 verified digest，唯一 workload signer 才可封存 receipt；verifier不能 claim、持久或自签。真实低基数 metrics 只能通过 production Queue/PageCommitter/Limiter/qualification dependency decorators 接线，不得复制 Worker loop或解析日志。`AdmitGate` 是唯一 gate-open persistence owner：它在同一 serializable transaction 内锁定并重载 Source/Revision、validation、current unexpired qualification/HA receipts 和 Provider-specific evaluator，复验签名、CAS、expiry 与全部 digest 后，才原子写 `AVAILABLE`、gate evidence pointer/digest、Audit 和 Outbox；任一缺失、过期、漂移、并发变化或签名失败整笔回滚并保持关闭。对需要资格的 Profile，stored `AVAILABLE` 只是必要条件，普通 `RequestSync`、Queue claim/heartbeat 与 PageCommitter 还必须用数据库时间重载同一 current unexpired evidence tuple；过期或漂移在任何 Catalog 写入前 effective fail closed。direct SQL、测试 fake、临时 bypass、普通同步成功或最终 Provider matrix 都不能替代该事务。

Gate epoch 采用闭合算术：起点 `G` 的首次发布路径为 Validation Run `G`、publication `UNAVAILABLE/G+1`、qualification receipts 保持该 gate epoch、atomic open `AVAILABLE/G+2`；显式可见验证路径为 Source `VALIDATING/G+1`、publication `UNAVAILABLE/G+2`、qualification receipts 保持该 epoch、open `AVAILABLE/G+3`；稳定 `AVAILABLE` 更新不得改变 epoch/binding；lineage rollover 则固定为 Run `G`、执行期 `DEGRADED/G+1`、terminal `AVAILABLE|SUSPENDED/G+2`。任何 epoch 跳跃、无治理事实的单独递增或旧验证/qualification 证明复用都拒绝。成功数据页、Validation 和 qualification 必须分别封存不可变 `DATA_PROJECTION`、`VALIDATION_PROOF`、`QUALIFICATION_PROOF` 后进入 `FINALIZING`，再以 Broker-owned opaque cleanup attempt 完成凭据/会话吊销；cleanup 不确定必须 `FAILED + SUSPENDED`，不能覆盖已经封存的 work result 或伪造成功。`Complete/Fail` 持久化由完整 terminal tuple 计算的 `terminal_command_sha256`，并写 exact `ASSET_SOURCE_RUN/TERMINAL_COMMITTED` receipt `request_id="source-terminal:<run_uuid>"`、`payload_hash=terminal_command_sha256` 后才销毁 raw fence；响应丢失只能按该 receipt 只读重放。Terminal Run、其 receipt、Validation Revision `VALIDATED|REJECTED`、qualification evidence closure 或数据 Source success/complete/gate closure 必须在同一 serializable transaction 原子闭合；cleanup `UNCERTAIN` 缺 Source `SUSPENDED` 时整笔回滚。
