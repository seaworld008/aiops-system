...
当前纠偏后的实现与合并顺序唯一为 `reachability docs corrective → manager exact-3 contract sync → source-gate capability-identity harness C0 → pre-A2a exact-2 routine/test-boundary corrective → manager exact-3 evidence sync → global routine ACL exact-11 contract + status sync → pre-A2a formal-fixture compatibility exact-9 corrective → fresh Task 19A2a exact-12 → post-A2a exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；每步只消费最新 `main` 已合并契约，PR/fixture/旧 dirty worktree 或未合并实现不得反向成为事实源。global routine ACL exact11 已由 PR #152 合并到 `main@d8d632f453929fbe056dc499a6d64144d83011bf`。capability-identity harness C0 只拥有 `.github/workflows/ci.yml`、`.env.example`、`scripts/with-local-postgres.sh`、`docs/operations/local-postgresql-development.md`、`internal/assetcatalog/postgres/migration_integration_test.go`、`internal/assetcatalog/postgres/recovery_container_test.go`、`internal/assetcatalog/postgres/migration_recovery_integration_test.go` 与 `internal/store/postgres/migrations_integration_test.go`，只预置隔离角色/DSN/证书、bidirectional future-gated application-DB ACL reconciliation和测试 identity，不授予函数或业务能力；只有 exact-38 owned manifest + exact-110 global ACL postflight进入 grant分支并随后通过 full admission，exact-36/down/unknown/partial都先 revoke并验 absent，unknown/partial再 fail closed。`.env.example` 同时预留空的 `AIOPS_SOURCE_GATE_ADMIT_DATABASE_URL` 与 `AIOPS_DISCOVERY_SOURCE_GATE_SEAL_DSN_FILE`，A2c 才装配。上一 fresh A2a 因 predecessor exact72 的默认 PUBLIC `EXECUTE` 形成144条额外 capability edge而停止；PR #152 后的 fresh A2a 又确认 pre-A2a fixture 会与 formal `000015` 内建的 exact3/trigger/drop 重复构造。两次 partial A2a 都是 `STOPPED/NOT PASS`，未提交 worktree/WIP/snapshot 永不作为输入。

pre-A2a formal-fixture compatibility exact9 corrective 的 Phase A 唯一拥有本轮八份权威文档，独立复核通过后 Phase B 才可唯一修改 test-only `internal/assetcatalog/postgres/migration_corrective_test.go`。fixture construction 只接受两个完整合法状态：predecessor 尚无 exact3 columns/closure trigger/down drop 时按原合同合成；formal A2a 已精确内建三者时验证并直接复用。partial、duplicate、renamed、wrong type、wrong trigger、wrong relation、wrong lifecycle、dynamic DDL 或 up/down 不一致全部 fail closed。该 corrective 不得收窄既有 adversarial matrix、global exact72/110、owned exact38、ACL/owner/grantor/grantability、C-order 或 down revoke/restore manifest；不修改 migration、ABI、OpenAPI、production code、完成度或能力状态。Task 19A2a exact12 仍禁止修改该 pre-A2a helper，只有 exact9 合并后才可从最新 `origin/main` fresh 启动；所有 Source/Provider/Worker 继续 `NOT_STARTED/UNAVAILABLE/CLOSED`。
