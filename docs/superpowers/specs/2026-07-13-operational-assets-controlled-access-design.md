...
所有 `*_digest` 值都只能是 lowercase 64-character SHA-256 hex；`qualification_signature` 只能是 canonical unpadded base64url，解码后重新编码必须逐字相等且不得含 `=`。`qualification_receipt_issued_at` 是 cleanup 后 receipt seal 的独立时刻，不能复制或复用 cleanup 前的 `work_result_recorded_at`；issued/expiry 都必须 finite，且 expiry 严格晚于 issued。
...
持久生命周期唯一为 `queue immutable binding → WorkResult → cleanup REVOKED → receipt seal → terminal closure → Task 19A2b AdmitGate`：queue 时 evidence kind、scope/binding/profile/runtime/lab/prior digests 已固定且不可变；WorkResult 只追加 result digest。cleanup proof 完成前 issued/expiry/key/signature/receipt digest 与全部 HA columns 必须为 `NULL`。取得 cleanup proof 后，同一个最终 `SERIALIZABLE READ WRITE` logical closure 才可原子记录 `REVOKED`、封存 receipt、按 evidence kind 写入 all-present `TWO_WORKER_HA` tuple 或 all-null `PROVIDER_CANARY` HA tuple、提交 terminal Run 和 exact `ASSET_SOURCE_RUN/TERMINAL_COMMITTED` receipt；此时 Source pointer 仍全 `NULL` 且 gate 保持关闭。Task 19A2b `AdmitGate` 是唯一另一个 serializable transaction：它重载并复验 current receipts/evaluator 后，才原子写三列 pointer、`AVAILABLE`、`run.gate_revision+1`、Audit 和 Outbox。唯一 immutable verifier registry/outcome sink reloads durable facts，verifier 不能 claim、持久或自签；真实 metrics 只能经 production decorators 接线。普通 `RequestSync`、Queue claim/heartbeat 与 PageCommitter 仍需用数据库时间重载 current unexpired tuple，过期或漂移在任何 Catalog 写入前 effective fail closed。

只有 migration-owner 可在 disposable `_test.go` database 中，为 closure/recovery 测试构造 structurally-valid final `AVAILABLE` fixture；synthetic pointer 与 `AVAILABLE` 必须位于同一最终 serializable transaction。该 fixture 不能成为 Task 19A2a/19A2b、G2/G4 或 availability 证据，也不能进入 production assembly。application/workload 对 gate/qualification/HA columns 的 direct write 必须返回 `42501`；测试 fake、临时 bypass、普通同步成功或最终 Provider matrix 都不能替代真实 receipt 和 `AdmitGate`。

实现与合并顺序唯一为 `docs-contract corrective → 修正/重基 PR #134 → merge PR #134 → fresh Task 19A2a → exact-2 validation corrective → Task 19A2b → Task 19A2c → Task 29A → Task 19B → Task 29B`；每步只消费最新 `main` 已合并契约，PR #134、fixture、旧 dirty worktree 或未合并实现不得反向成为事实源。post-A2a exact-2 是只顺序修改 `internal/assetcatalog/validation.go`、新建 `internal/assetcatalog/validation_test.go` 的 domain-validation C0，不得修改 migration、Repository、Queue、HTTP、Worker、signer 或 config。该字符串是 successor contract，不声明完成度；本 corrective 合并后，主管理窗口必须从最新 `main` 另开 exact-3 manager-only PR，只修改 `docs/status/current.md`、`docs/superpowers/plans/2026-07-13-governed-operations/coverage-matrix.md`、`docs/superpowers/plans/2026-07-13-governed-operations/01-assets/09-discovery-worker-ha-e2e.md`，把同一顺序与 `Consumes` 发布到全部必读入口，不改 ABI、完成度或 checkbox。该 exact-3 合并前 admission 停在 docs corrective，任何 PR #134 修正/merge 或 A2x/29x 实现不得启动；manager-only 发布不是新增实现 Batch，也不改变上述顺序。
