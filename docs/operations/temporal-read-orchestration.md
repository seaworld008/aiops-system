...
阶段：M5C2-4b（版本化只读编排与 Runner Activity 契约；尚未进行进程装配，READ claims 关闭）
...
- Runner queue：`aiops-investigation-read-task-v1-<environment UUID>-<bundle>`。
...
control queue 同时绑定完整 Plan manifest、connector registry 与 atomic Bundle 摘要；Runner queue 再绑定
精确 Environment UUID，防止另一环境或不兼容 Bundle 的 Runner 拾取 Task。旧、新 digest Worker 必须按
各自 exact queue 共存到对应 Workflow 和 Task 全部终态，不能让请求正文选择 queue 或 Bundle。
...
`readruntime.Bundle`，以及非空、服从 context 的可信 `readexecutor.BearerSource`。测试 fake 只能通过包内
私有端口使用；没有 anonymous、静态 token、credential-free 或 WRITE fallback。

任何 Gateway 请求前，Activity 必须验证 exact Workflow/Activity ID、namespace、run ID、Environment/
Bundle queue、单次 attempt、timeout/retry/priority，以及输入 Bundle digest 与进程 Bundle 完全相等。
