...
日期：2026-07-11
阶段：M5C2-3a/C2-3b（内容寻址 target/binder 与未装配的固定 READ HTTP executor；READ claims 关闭）
...
只有可信 `ExecutionStart` 的 epoch/scope revision 再次一致后，`Execute` 才能消费它一次。
执行结果继续绑定同一 task/epoch/scope revision；只有同一个 `ExecutionStart` 与匹配
task/epoch 的 Fence 才能生成 Completion，跨任务/attempt/scope 交换结果会 fail closed。

该 executor 尚未接入 `cmd/read-runner`、Worker、Gateway callbacks 或 Outbox dispatcher；本阶段
不新增 route、配置、迁移或 Activity 注册。Control Plane 的 `ClaimsEnabled` 仍为 `false`。
...
- DNS/dial 5 秒、TLS handshake 5 秒、response header 10 秒、上游 query timeout 10 秒、总请求
  ...
  因此 C2-4 只能装配同步、服从 context 且对远端取凭据设置更短超时的可信 provider；
...
C2-3b 的本地 TLS 契约与负向测试只证明代码契约，不等价于企业 PKI、真实数据源、Vault role、
ServiceAccount、NetworkPolicy 或 egress enforcement 验收。C2-4 才负责原子装配配置、READ Runner、
