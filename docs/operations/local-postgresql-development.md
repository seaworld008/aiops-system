...
Source Gate A2a additionally consumes two production capability identities：`aiops_source_gate_sealer` and `aiops_source_gate_admitter` are both `LOGIN NOINHERIT` with the same five dangerous flags disabled、no membership、mutually distinct credentials/DSNs，and no base runtime or extension-owner rights。Their application database/schema ACL is absent at the predecessor72/owned36 state。Only a future trustworthy `000015` owned exact-38 plus application-schema global exact-110 postflight may enter the conditional grant branch for direct database `CONNECT`、schema `USAGE` and each identity's own single function `EXECUTE`；down/unknown/partial always revokes it。They have no relation/sequence ACL and cannot call one another's routine。

The current exact-36 `DatabaseRoleAdmission.Check` remains the four-base-role application probe and requires `session_user=current_user='aiops_control_plane_workload'`；the capability harness exact8 does not modify it，and instead uses control/admin fixtures to prove the two capability identities、ACL absence、pairwise-distinct credentials/DSNs and application-connection rejection。Formal A2a alone extends the application probe to predecessor72+owned36 normalized ACL or global110+owned38 hardened ACL。Pre-up predecessor has no direct grant and uses normalized PUBLIC72；post-up uses runtime→predecessor exact72 owner-grantor/non-grantable direct edges。The only C-order identities and production digest are fixed by [Pack06 canonical predecessor exact72 runtime EXECUTE manifest](../superpowers/plans/2026-07-13-governed-operations/01-assets/06-source-external-cmdb.md#canonical-predecessor-exact72-runtime-execute-manifest)；local tests must match every entry and constant，never derive a live snapshot and accept it。Workload direct0/effective72 through runtime；sealer/admitter predecessor0。Capability probes run only forglobal110+owned38 and reject role switching、cross-function or relation ACL。Until those gates pass，both capabilities remain`CLOSED/UNAVAILABLE`。Migration identity and role-switch admission remain separate before`BEGIN`。
...
The wrapper reads each password only into process memory, URL-encodes all URI components through standard input, constructs and exports the five DSNs above, proves all credentials are pairwise distinct, runs each positive TLS/certificate-serial identity probe and its three dual-factor negatives, unsets the plaintext shell variables, and then `exec`s the requested command. Probe failures suppress libpq output and report only the fixed identity name；the wrapper never prints or writes a password or DSN. Tests must use the identity-specific variable; `AIOPS_TEST_POSTGRES_DSN` is retained solely as the control/admin harness variable while legacy fixtures are migrated. The pre-A2a helper may create roles/credentials and control-database login evidence, but its application-database ACL reconciliation is exact and bidirectional：only trustworthy owned exact-38/global exact-110 postflight enters the conditional capability `CONNECT|USAGE` grant branch；predecessor72/owned36、successful down、unknown or partial state enters revoke-and-prove-absent，with unknown/partial returning closed after revocation。A2a must preserve the merged global helper，and its exact-12 integration test must prove pre-up72、runtime exact72 edge count/hash、up110 direct/effective/PUBLIC/owner/grantor/grantability、unexpected111、wrong predecessor ACL、down restored72、up/down/up、all five identities and application-identity key DML/trigger/four-helper behavior。Because ACL/archive restoration semantics change，dual-instance dump/restore recovery is required A2a G2 rather than deferred and must prove restored runtime exact72 edges plus down-time exact revoke/PUBLIC72 restoration。No harness may use schema-wide revoke/grant，and every future `000016..000022` public routine must update the content-addressed global manifest、explicit PUBLIC revoke and rollback contract。
