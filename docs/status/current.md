...
> 当前集成基线：最新 `origin/main@eb3080231c2dc501884fd797f7f6f1bffacc1a23`（PR #92）；PR #90、#93、#91、#92 已合并，CSV Profile Registry/runtime admission 与 External CMDB Task 17 fixed-protocol foundation 为 `BUILT_CLOSED`。当前两个产品构建 Batch 为 Pack 06 Task 18A External CMDB paging/checkpoint protocol foundation 与 Pack 07 Task 20 vSphere fixed client/validation foundation；另有一个处理两条 merge-late P2 并补齐 client typed backoff 合同的 Review corrective，三者在 PR 合并前均只记为 `BUILDING_CLOSED`
...
上述 merge 后又到达两条经主管理复现仍有效的 P2，当前保持 OPEN：[PR #91 discussion_r3600742887](https://github.com/seaworld008/aiops-system/pull/91#discussion_r3600742887) 要求非 `MANUAL_V1` validation runtime 默认关闭；[PR #92 discussion_r3600811950](https://github.com/seaworld008/aiops-system/pull/92#discussion_r3600811950) 要求 caller cancel/deadline 贯穿 External CMDB client 与 Validate，不能投影为 Provider failure。owner 为独立 Review corrective，精确五文件边界为 `internal/assetcatalog/postgres/source_revisions.go`、`internal/assetcatalog/postgres/source_revisions_test.go`、`internal/assetcatalog/postgres/source_revisions_integration_test.go`、`internal/assetsource/externalcmdb/client.go`、`internal/assetsource/externalcmdb/client_test.go`。新增的 integration test 所有权只用于把两条旧 `EXTERNAL_V1` validation 真库断言纠正为 `ErrUnavailable` 且零副作用；这是 MANUAL-only 安全合同引起的旧测试漂移，不放宽任何生产 runtime。修复、测试和合并证据均尚待，threads 继续 OPEN；不追溯改写 PR #91/#92 的 `BUILT_CLOSED`，相关运行能力继续关闭。
...
| 新资产目录与发现 | BUILT_CLOSED（M0/M1A/M1B/M1C/M1D/M1E0/M1C1/M1E/Queue/CleanupBroker/Limiter C0/M1F/M1G/M1H/M1J/Session C0/Credential Trace C0/TypeScript C0/M1I Web Foundation/Source Revision/Create/Profile Registry/M1I Asset Catalog UI/Mapping Workbench/Navigation Scope corrective/Task 14 Source API/Task 15A CSV parser/CSV Profile Registry+runtime admission/Task 12 Source inventory/External CMDB Task 17）/ BUILDING_CLOSED（External CMDB Task 18A/vSphere Task 20/Review corrective）/ UNAVAILABLE | CSV Registry 与 Task 17 fixed-protocol foundation 已合并，但 CSV/CMDB runtime 均关闭；Task 18A 和 Task 20 只构建 Provider-side foundation，Review corrective 处理两条 OPEN P2 与 Task 18A client typed backoff 前置合同。Task 15B、完整 CMDB PostgreSQL/Worker 闭环、Discovery Worker、真实 Provider gate 与运行资格仍未完成 |
...
从最新 `origin/main@eb3080231c2dc501884fd797f7f6f1bffacc1a23` 并行执行以下两个产品构建 Batch；两者均已创建可见隔离窗口但尚未形成已合并 PR，因此只记录为 `BUILDING_CLOSED`，不记录 commit、G1/G2 通过或完成事实：

- Pack 06 Task 18A External CMDB paging/checkpoint protocol foundation：只在已合并 Task 17 窄 Provider 上构建 `CAPABILITIES → ASSETS → RELATIONS → COMPLETE` 固定分页、私有 canonical checkpoint、真实 TLS wire fixture 与 bounded `Page|Delay` Provider-side 协议；合同预检发现现有 client 把 `429/503/Retry-After` 折叠丢失，无法在原八文件边界内安全构造 `Delay`，故窗口已 clean 停在 `WAITING_ON_CLIENT_CORRECTIVE`，当前无 diff、RED 或 PR。它不修改 PostgreSQL PageCommitter，不装配 Queue/Worker/fence，不执行缺失检测、恢复或真实 gate，也不代表完整 Task 18。
- Pack 07 Task 20 vSphere fixed client/validation foundation：只构建 exact `VSPHERE_VCENTER_V1` fixed client、vCenter identity/privilege validation 与安全 normalization foundation；不注册 Source Profile，不实现 Task 21 full/delta checkpoint、Task 22 canary/gate 或 Worker。PR 合并前 vSphere 主线事实仍为 `NOT_STARTED/UNAVAILABLE/CLOSED`。

另有一个不阻塞上述文件无重叠 Batch 的 Review corrective 正在处理两条 OPEN P2，仅允许 MANUAL validation runtime 白名单与 External CMDB caller cancellation 贯穿；同一 `client.go/client_test.go` owner 还负责补齐 Task 18A 所需的 package-private typed `429/503/Retry-After` backoff 合同，并只在前述新增的 integration test 文件纠正安全合同导致的旧真库断言漂移。在 corrective 合并前只记为 `BUILDING_CLOSED`，不记录完成证据；Task 18A 必须继续等待。主管理会话负责修复合并后处理 threads，本状态同步不修改 thread。
...
External CMDB server-installed registry/creation admission、Task 18 完整 PostgreSQL/Worker/fence/缺失检测/恢复闭环、Task 19 真实 staging canary/per-source availability gate，以及 vSphere Task 20–22 完整闭环和所有真实 Provider 资格仍保持 `NOT_STARTED/UNAVAILABLE/CLOSED`。
