...
> 当前集成基线：本文件所在的最新 `origin/main`；最近完成 Batch：`M1K-credential-problem-trace-corrective`（PR #70，代码提交 `113af87`）；当前工作：`M1I-typescript-toolchain-contract-corrective`，Web Task 9 在 clean 边界暂停
...
M1I Web Task 9 的前置工具链核验发现 C0 权威版本冲突：规划固定 TypeScript `7.0.2`，但固定 `typescript-eslint@8.63.0` 正式 peer contract 只接受 `>=4.8.4 <6.1.0`，固定 `openapi-typescript@7.13.0` 只接受 `^5.x`；截至 2026-07-17 的 npm 最新稳定版本仍未提供 TypeScript 7 peer 支持。原任务已删除本轮三个未跟踪半成品并恢复 clean，未进入业务实现、未提交。当前独立 corrective 正在以正式 peer intersection TypeScript `5.9.3` 为候选，统一修正版本基线与重复规划事实并完成独立复核；在该契约合并前不得恢复 Task 9 或用忽略 peer、双 TypeScript、手写 DTO 等方式绕过。
...
- 前端应用平台架构已纳入确认规划：唯一 `web/` 采用 React + TypeScript + Vite 8 + TanStack，固定 `app → features → shared`、OpenAPI 唯一 DTO、服务端 `effective_actions`、Go 同源 SPA/API 与单 Control Plane 镜像 `/opt/aiops/web`；智能体验只通过 Evidence/Proposal/ActionPlan/Operation/Audit 受治理链呈现。`M1I-web-foundation-assets` Task 9 因 TypeScript/tooling peer contract 冲突在 clean 边界暂停，当前仍为 `BUILDING_CLOSED` 且不可用。
...
| 新资产目录与发现 | BUILT_CLOSED（M0/M1A/M1B/M1C/M1D/M1E0/M1C1/M1E/Queue/CleanupBroker/Limiter C0/M1F/M1G/M1H/M1J/Session C0/Credential Trace C0）/ M1I BUILDING_CLOSED / UNAVAILABLE | Session OpenAPI 与 Credential Problem trace corrective 已合并；Web Task 9 等待工具链契约 corrective；Source mutation、Worker、前端与真实 Provider 门仍未完成 |
...
| 新 React 前端 | BUILDING_CLOSED | Web Task 9 在 clean 边界暂停，等待 TypeScript/tooling peer contract corrective 合并后从最新主线重开；未验收前仍不可用 |
...
先完成 `M1I-typescript-toolchain-contract-corrective`：以 npm 正式 peer contract 证明并统一权威版本基线、Task 9 exact pin 和全部重复规划事实，完成独立 P0/P1 复核后提交 PR。不得通过忽略 peer dependency、package extension、第二套 TypeScript 或手写生成 DTO 制造绿灯。

corrective 合并并验证远端主线后，从最新 `origin/main` 新窗口重新执行 `M1I-web-foundation-assets` Task 9：消费唯一 OpenAPI 的 `getBrowserConfig/getSession` 与现有 authenticated handler，建立 `web/` 工具链、生成类型、OIDC/Scope application shell、共享 API/UI 和 Go same-origin SPA service。它不得重新修改 OpenAPI、Session handler、migration 或 status；完成 G2 后再提交 PR，并在合并后轮换后续窗口。
