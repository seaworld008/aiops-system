...
上述 merge 后又到达两条经主管理复现仍有效的 P2，当前保持 OPEN：[PR #91 discussion_r3600742887](https://github.com/seaworld008/aiops-system/pull/91#discussion_r3600742887) 要求非 `MANUAL_V1` validation runtime 默认关闭；[PR #92 discussion_r3600811950](https://github.com/seaworld008/aiops-system/pull/92#discussion_r3600811950) 要求 caller cancel/deadline 贯穿 External CMDB client 与 Validate，不能投影为 Provider failure。owner 为独立 Review corrective，精确只拥有 `internal/assetcatalog/postgres/source_revisions.go`、`source_revisions_test.go` 与 `internal/assetsource/externalcmdb/client.go`、`client_test.go`；当前尚无合并修复、测试或 resolved 证据，不追溯改写 PR #91/#92 的 `BUILT_CLOSED`，相关运行能力继续关闭。
...
另有一个不阻塞上述文件无重叠 Batch 的 Review corrective 正在处理两条 OPEN P2，仅允许 MANUAL validation runtime 白名单与 External CMDB caller cancellation 贯穿；同一 `client.go/client_test.go` owner 还负责补齐 Task 18A 所需的 package-private typed `429/503/Retry-After` backoff 合同，不扩文件边界。在 corrective PR merge 前只记为 `BUILDING_CLOSED`，不记录测试或 resolved 事实；Task 18A 必须继续等待。主管理会话负责 merge 后回复/resolve，本状态同步不修改 thread。
